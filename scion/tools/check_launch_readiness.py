...
def _prompt_context_readiness_check(root: Path) -> tuple[str, Any]:
    ...
    if spec is not None:
        live_markers[spec.active_subject_marker_group_name] = {
            ...
            for name, (relative_path, marker) in (
                ACTIVE_SUBJECT_CODE_CONSTRAINT_PROMPT_MARKERS
                | spec.active_subject_provider_markers
            ).items()
        }
...
def _problem_measurement_diagnostics_prompt_summary_failures(
    ...
    *,
    ...
    payload = value if isinstance(value, dict) else {}
    spec = _problem_prompt_bridge_spec(problem_family)
    if spec is None:
        return [{"reason": f"{failure_prefix}_prompt_bridge_spec_missing"}]
...
def _resolve_problem_v1_path(
    *,
    ...
    if spec is None:
        return None
    return resolve_problem_v1_path(
        root=root,
        manifest=manifest,
        repo_dir=REPO_DIR,
        spec=spec,
