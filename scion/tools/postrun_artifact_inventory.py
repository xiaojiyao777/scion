...
CVRP_PREPARED_HANDOFF_PORT = CvrpPreparedHandoffReviewPort()
WAREHOUSE_PREPARED_HANDOFF_PORT = WarehousePreparedHandoffReviewPort()
...
def _add_cvrp_measurement_handoff_checks(
    ...
    *,
    ...
    checks = CVRP_PREPARED_HANDOFF_PORT.prepared_contract_checks(
        manifest,
    ...
    for name, item in checks.items():
        add_check(name, item.get("passed") is True, item.get("detail"))
...
def _add_warehouse_followup_handoff_checks(
    ...
    checks = WAREHOUSE_PREPARED_HANDOFF_PORT.prepared_contract_checks(manifest)
    for name, item in checks.items():
        add_check(name, item.get("passed") is True, item.get("detail"))
...
def _problem_specific_phase4_requirements(manifest: Any) -> dict[str, Any]:
    ...
    if manifest.get("problem_family") == "cvrp":
        return CVRP_PREPARED_HANDOFF_PORT.phase4_requirements(
            manifest,
            _coverage_item,
        )
    if manifest.get("problem_family") == "warehouse_delivery":
        return WAREHOUSE_PREPARED_HANDOFF_PORT.phase4_requirements(
            manifest,
            _coverage_item,
        )
    return {}
