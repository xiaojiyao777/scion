...
- The next CVRP campaign is temporarily blocked by LLM infrastructure, not by
  ...
  artifacts from being treated as current-run postrun evidence. Inventory
  top-level branch/event/hypothesis/LLM-trace fields are current-run scoped;
  copied campaign counts live under `resume_snapshot`. For historical roots or
