...
- Strict launch readiness for both current roots reports `static_ready=true`,
  ...
  were prepared from WSL runtime commit `a5894a66`. Current
...
- Current launch/runtime boundary: prepared roots must guard `scion/tools`,
  ...
  report/readiness bundle, if the `SCION_DIR` missing pre-campaign failure path
  skips the same report/readiness bundle, or if dirty/head-mismatch runtime
  guard failure paths skip the same report/readiness bundle. Both warehouse and
  CVRP roots also fail readiness if
