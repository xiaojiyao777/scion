...
19. **R66 original action and recovery1 are terminal infrastructure failures;
   recovery2 is a terminal H observation with old-mechanism overlap:** the
   ...
   lazy chat dependency seam without HTTP. Its terminal status is now
   `TERMINAL_HYPOTHESIS_OBSERVATION`. The earlier broad, source-send
   and recovery1 instructions remain preserved, and the exact new
   `recovery2_source_send_authorization_text` is recorded below:
   ...
   This overrides the prior no-recovery2 stop only for one fresh recovery2 H
   action. The authorization-bearing refreeze and independent checks below
   passed before launch. C, solver, Decision, promotion and R67 remain zero.
   Its authorization-bearing
   refreeze receipt is 4,200 files/73,527,199 bytes, tree SHA-256
   `a3178c2d...`, manifest SHA-256 `b91bbec4...`, and 47 passing tests in 12.19
   seconds plus runner/style/shell/JSON checks. The exact 1,460-byte UTF-8
   authorization has SHA-256 `25198479...`; the launch sentinel equals the
   exact recovery2 label. Output, control and socket roots were absent at that
   refreeze, which sent no provider payload and did not launch.

   The launch-before documents were committed as `a0a005a6`. Recovery2 then
   returned `LAUNCHED inner_pid=655763` and durably acquired at
   ...
   contents and every H response, observation, classification and terminal
   artifact were unread. Marker existence alone did not establish completed
   HTTP or a provider return.

   Recovery2 subsequently exited `0`. Result/status are 7,039/842-byte mode-
   `0600` files with SHA-256 `19e01061...`/`d6d359b7...`; wrapper/H/actual-
   network counts are `1/1/1`, terminal response true, retry zero, latency
   16,134 ms and all six mechanical fields true. The one typed H proposes a
   deadline-polled bounded intra-route 3-opt neighborhood after
   `_two_opt_intra`. A read-only post-terminal audit leaves the durable primary
   record and null scientific classification untouched, but labels it only
   `DESCRIPTIVE_DIRECT_OLD_BOUNDED_INTRA_ROUTE_3OPT_OVERLAP`: successor5 was
   weak below MDE, successor6 expanded the same family to `18W/22L/8T`, median
   `0`, summary `-0.75`, then abandoned it, and activated R3 `three_opt_intra`
   was also soft-abandoned on negative median evidence. The
   [recovery2 postrun](docs/experiments/v0.4/v0.4-cvrp-r66-h-only-mechanism-frontier-probe-recovery2-postrun-20260815.md)
   seals `NO_NOVEL_FRONTIER_EVIDENCE / ZERO_ALGORITHM_QUALITY_EVIDENCE` and
   `NO_C / NO_R67 / NO_RECOVERY3`. All downstream counters are zero; the inner
   PID is gone, while the live tmux server/socket remain retained and must not
   be deleted.
...
| R66 | original `PREP_INVALID`; recovery1 terminal local-infra failure before HTTP; recovery2 terminal H observation | Original stopped on frozen `64` versus wall-clock `65` date drift with zero calls. Recovery1 emitted provider failure at logical `1/1`, but audited cause is local missing `jiter` before HTTP. Recovery2 corrected only that dependency, completed exit `0` with wrapper/H/network `1/1/1`, terminal response true and all six mechanical fields true. Its typed H is bounded deadline-polled intra-route 3-opt after `_two_opt_intra`. | Preserve the primary `TERMINAL_HYPOTHESIS_OBSERVATION` and null scientific classification. A separate non-gating audit labels `DESCRIPTIVE_DIRECT_OLD_BOUNDED_INTRA_ROUTE_3OPT_OVERLAP`; seal `NO_NOVEL_FRONTIER_EVIDENCE / ZERO_ALGORITHM_QUALITY_EVIDENCE`. Every downstream/R67 counter is zero: no C, candidate, solver, Decision, recovery3 or promotion. |
...
seconds. The provider-free A/A/null calibration completed acceptably under its
  ...
  roots. The user subsequently supplied exact one-action recovery2 authority,
  overriding the prior no-recovery2 stop. Recovery2 completed exit `0` as
  `TERMINAL_HYPOTHESIS_OBSERVATION`, with exactly one actual network request,
  terminal response and a mechanically complete bounded intra-route 3-opt H.
  Historical comparison directly overlaps old bounded 3-opt evidence, so the
  non-gating research seal is `NO_NOVEL_FRONTIER_EVIDENCE`; algorithm-quality
  evidence is zero and no C, R67 or recovery3 follows.
