...
(`45 passed` and `39 passed`), launch/postrun broad suite (`264 passed` after
commit `883b2e9a`), plus `py_compile` and `git diff --check`.
...
resume context. Running and preflight-failure launcher status preserve the
...
acceptance still passes (`85 passed` each). Problem-summary actionability
policy is now problem-owned through `ProblemSummaryActionabilitySpec` in the
...
`postrun_artifact_inventory.py` is now a thin CLI/Markdown wrapper; inventory
loading lives in `scion.postrun.inventory.loader`, while problem-owned default
ports and legacy problem launcher status extensions live in
`scion.problems.postrun_inventory`. JSON keys, Markdown rendering, and
launch-readiness behavior are preserved through this adapter boundary. The
generic loader is source-guarded against problem-family vocabulary and keeps
CVRP/warehouse/VRP semantics out of `scion.postrun.inventory`. Local focused
validation passes for postrun inventory/brief/acceptance/rebuild and v3
boundary suites (`163 passed`), strict readiness passes on the successor6 root,
and the only launch-readiness focused failures before commit are the expected
dirty-worktree runtime guard checks. Prepared
prompt-context focus signals for CVRP and warehouse now also come from those
problem-owned ports; `rebuild_prepared_handoff.py` merges their signal payloads
and keeps only the common prompt-context artifact orchestration. Prepared
prompt bridge metadata is problem-owned as well:
...
CLI/file-output orchestration rather than owning readiness semantics. The
second prompt-context split moves CVRP and warehouse active-subject and
measurement diagnostics prompt-summary semantics into
`scion.problems.cvrp.prompt_bridge` and
`scion.problems.warehouse_delivery.prompt_bridge`; generic handoff code now
dispatches through `ProblemPromptBridgeSpec`, and
`scion.postrun.handoff.prompt_context_readiness_validation` owns the prepared
artifact validation used by launch readiness.
...
`used`, activation/objective/phase evidence observed in 2 of 2
Protocol rows, positive-at-MDE absent, and postrun CMT protection evidence
complete after the report-only case-delta projection repair. Rebuilt
research-efficiency top rows now carry bounded `case_level_deltas` from public
raw-metrics refs; the CVRP review observes CMT2/CMT4 `total_distance` deltas
and reports `evidence_requirement_statuses.status=complete`,
`cmt2_cmt4_case_protection.status=observed`, and
`required_evidence_proof.checklist_status=proven`. Postrun acceptance on this
root reports `current_run_analysis_ready=true` and no failed required checks.
The run remains solver-negative: champion `v1`, promotions `0`, rows at or
above MDE `0`. The run also exposed a final-status metadata gap: postrun
inventory could recover `resume_snapshot_ref` from the prepared manifest, but
final root `run_status.json` dropped it after campaign status copy. The final
wrapper status writer now preserves resume metadata for future roots.
...
- Design N problem-summary actionability policy is problem-owned:
  `scion.postrun.ProblemSummaryActionabilitySpec` defines the legacy summary
  key, schema, supported delegated interpretations, launch-required field, and
  blocking/nonblocking evidence-gap policy. CVRP large-twoopt and warehouse
  follow-up specs live beside their problem-owned summary providers, and the
  checker derives compatibility schema/actionability detail from those specs.
...
- Design N prepared-contract inventory migration is present:
  ...
  Inventory loading is now package-owned under `scion.postrun.inventory`:
  constants, lifecycle, evidence coverage, traces, database readers, prepared
  ports, and the public loader are split into cohesive modules, all below the
  1000-line warning threshold. `postrun_artifact_inventory.py` is a thin
  CLI/Markdown wrapper, and `scion.problems.postrun_inventory` supplies the
  CVRP/warehouse default ports plus legacy problem launcher status extensions.
  JSON keys, Markdown rendering, and launch-readiness behavior remain
  compatible without storing problem semantics in the generic inventory
  package. The generic source is guarded against CVRP/warehouse/VRP vocabulary;
  focused local tests pass for the new loader modules, legacy artifact
  inventory behavior, launchers, postrun brief, and problem review providers.
  Prepared prompt-context focus signals
  for both problem families are now generated by the same problem-owned ports;
  `rebuild_prepared_handoff.py` keeps artifact orchestration and common
  decision-boundary readiness, not problem-specific research-focus parsing.
  Prepared prompt bridge metadata now lives in
  `scion.problems.cvrp.prompt_bridge` and
  ...
  A follow-up prompt-context boundary split moves CVRP/warehouse active-subject
  and measurement diagnostics prompt-summary builders into problem-owned
  `prompt_bridge` modules, keeps `prepared_prompt_context.py` to neutral
  research-focus/research-shape summaries, and makes launch readiness call the
  named package validator `prompt_context_readiness_validation` instead of
  carrying a tool-local artifact helper block. Focused local validation passes
  for source-boundary, rebuild, and problem ports (`12 passed`) plus
  py_compile and `git diff --check`; the dirty-sensitive postrun artifact
  inventory plus launch-readiness suite passes after commit (`134 passed`).
...
- Superseded WSL evidence roots retained for lineage reference:
  ...
  - Target-intent authority resolution is implemented in the current worktree:
    ...
    Local head `ac33df06` and the synchronized WSL semantic-tightening head
    passed focused target-intent/proposal tests (`121 passed`). The validation
    root launched earlier from WSL head `542d1f99`
    `/home/xjy-ubuntu/research/scion-experiments/v04-cvrp-authority-542d1f99-postweakpressure-4r-gpt55-20260623T055230Z-claw`.
    The root finished current-run-ready with wrapper/postrun exit `0`, postrun
    acceptance `ready`, validity `valid`, completeness `complete`, 4 of 4
    effective Protocol rows, 0 proposal quality blocks, 0 active-slot blocks,
    and `last_stop_reason=max_rounds_exhausted`. It accepts Design G as a
    generic proposal-control repair: live `exploit_weak_positive` follow-up
    reached branch-local target intent, formal hypothesis/code generation, and
  ...
  - CVRP solver-depth old-checkout root
    ...
    finished from WSL commit `65115459` after strict launch readiness passed,
    resuming from the accepted authority validation campaign. It is
    current-run-ready, valid, and complete, with 6 of 6 effective Protocol rows,
    but it is framework/integration evidence rather than solver progress:
