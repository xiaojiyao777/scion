...
- Strict launch readiness for both current roots reports `static_ready=true`,
  ...
  true. The roots were prepared from WSL checkout `aa54418` after
  `check_postrun_acceptance.py` began requiring current-run problem-summary
  boundary markers.
...
- Current launch/runtime boundary: prepared roots must guard `scion/tools`,
  ...
  Older prepared roots before the postsummary roots above are not current
