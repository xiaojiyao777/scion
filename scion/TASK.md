...
- Strict launch readiness for both current roots reports `static_ready=true`,
  ...
  true. The roots were prepared from WSL checkout `d14f395` after launch
  ...
  `launch.env` to be exported before campaign start.
...
- Current launch/runtime boundary: prepared roots must guard `scion/tools`,
  ...
  `PYTHONPATH` before campaign start.
  Older prepared roots before the pythonpath roots above are not current
