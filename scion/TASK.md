...
- Current framework guarantees, all report-only/control-plane or problem-owned
  ...
  - Launch readiness guards the active checkout, absolute WSL `SCION_DIR` /
    ...
    private `launch.env` permissions, no-early-stop semantics, strict postrun
    rebuild/readiness, committed runtime-guard drift, and wrapper/manifest
    runtime-guard contract consistency.
