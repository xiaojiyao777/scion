...
- The current blocker is external WSL `gpt-5.5` provider auth, not Scion static
  ...
  `code=invalid_api_key`; latest auth pool was `active=0`, `expired=1`,
  `total=1`.
...
Both active prepared roots were generated on WSL from runtime commit
`cf8fb5a7` after the solver-source read headroom repair. Static readiness
