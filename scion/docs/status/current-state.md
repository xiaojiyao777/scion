...
- CVRP remains solver-negative. Successor19 and successor20 prove repaired
  ...
  Successor41 completed valid/complete/postrun-ready on
  ...
  screening rows, and normal model/tool accounting. Both rows screened 48/48
  pairs with zero failed pairs but stayed below MDE: row 1 had `17/17/14`,
  median `0.0`, mean `-0.12`; row 2 had `12/15/21`, median `0.0`, mean
  `0.56`. P/B/E-family losses remained. CMT2 was present in the split manifest
  but was not forced into the measured case set, so the next formal screening
  must force CMT2/CMT4 protected-case coverage or record an explicit
  measurement caveat. Treat `route_skeleton_regret_repair` as
  diagnostic-exhausted reviewed/default-avoid evidence; do not long-run,
  threshold-tune, rerun, or continue the same mechanism as a v0.4 optimization
  candidate. The postrun report is
  `scion/docs/experiments/v0.4/v04-cvrp-successor41b-route-skeleton-diagnostic-postrun-20260706.md`.
  The next CVRP slot should keep the causal-path gate, make the exact
  `material_difference.changed_dimensions` / `contrast` / `evidence` schema
  prominent, and then clean-fork to a materially different CVRP-owned causal
  path. The successor42 prelaunch repair design is
  ...
  `priority_case_ids`, CVRP formal screening declares CMT2/CMT4 as those
  priority cases, and raw metrics record configured/requested/effective
  priority-case coverage. This is a research-entry/protocol repair, not a
  solver mechanism.
...
- Successor44c implemented the attribution-contract repair in commit `2bcf831a`
  ...
  did not fake successor44 direct-effect `record_move` deltas.
- Successor44d implemented the CVRP-owned policy-effect warning hygiene repair
  in commit `150ab7de` and completed a valid two-row server-local screen:
  ...
  It used local `gpt-5.5`, passed completion preflight, made 7 LLM calls, had
  zero proposal-quality blocks and zero telemetry failures. The repair worked:
  branch-card and telemetry-guard summaries now keep
  `mechanism_contract_status=policy_outcome_observed`, no repair ids, no
  telemetry warnings/failures, and no generic `record_move(delta=...)` advice
  for this no-direct-effect policy mechanism. Solver evidence remained
  weak-positive screening only: the first row was `32/32` valid with W/L/T
  `14/7/11`, raw pair median `0.0`; the expanded row was `48/48` valid with
  W/L/T `27/14/7`, raw pair median `+3.0`, branch CI `[0.0, 7.25]`,
  `runtime_confidence=low_cached_champion`, CMT4 neutral, and B-family
  regressions. Do not long-run unchanged successor44d; next design must either
...
- Successor47 is complete and reviewed/default-avoid:
  ...
  `12/11/5`, row medians `0.0`, current-head median `0.5`, CI
  `[-6.0, 4.5]`, and no promotion. Mechanism runtime was observed, but direct
  positive mechanism best delta appeared in only `1/64` expanded-row candidate
  runs, and CMT2/CMT4 were unsafe. Trace audit found no fatal prompt
  truncation, model-call, target-binding, or v3-boundary failure. Park
  contiguous giant-tour split recombination for v0.4; do not long-run,
  threshold-tune, or create an unchanged successor47b. Postrun:
...
- Successor48 is now designed as the next materially different CVRP-owned
  ...
  `policies/baseline_modules/route_pool_recombination.py`, with minimal
  scheduler wiring only. It should build a small ephemeral route pool from
...
- Successor51 is now complete from committed repair `7dfbd1b0` on the
  ...
  It completed valid/complete/postrun-ready with local `gpt-5.5`, six
  successful LLM calls, one pre-code proposal-quality block, two effective
  screening rows, and zero promotions. The successor50 slot repair worked:
  an under-specified `bounded_edge_path_relinking` hypothesis was blocked
  before code generation, and the accepted retry generated a real solver
  mechanism, `bounded_route_arc_lns_rebuild`, in a new module with minimal
  scheduler wiring. Solver evidence stayed active-marginal below MDE: row
  medians were `0.0` and `1.0`, both CI highs were `5.0 < 9.9` MDE, and
  CMT2/CMT4 were objective-negative despite strong route-arc LNS phase
...
- Successor53 is now complete/invalid from committed guidance repair `7683d05f`
  ...
  evidence, not solver evidence. All four target-intent calls selected the
  right line, `protected_candidate_trajectory_selector` in `scheduler.py`, but
  all four hypotheses were blocked by `cvrp_solver_design_causal_path_contract`;
  attempts 2-4 only missed `algorithmic_intervention_sufficiency`. The root
  cause is a CVRP-owned gate/prompt mismatch: the hypotheses described the
  intended RNG-isolated protected trajectory selector, but the gate was too
  brittle around attempt/reject telemetry wording (`move_attempts`,
  `accepted_moves`, reject causes, `budget_exhausted`). The current checkout
  now broadens the CVRP-owned outcome-observation vocabulary and makes the
...
- Successor54 is now complete from committed gate-wording repair `d7a1370c`
  on the server-local `claw` runner:
  ...
  It completed valid/complete/postrun-ready with local `gpt-5.5`, eleven
  successful LLM calls, one pre-code quality block, two effective screening
  rows, and zero promotions. The successor53 gate-wording repair worked: the
  first hypothesis was blocked only for missing
  `branch_lesson_usage.clean_fork_diversity_claim`, and the retries reached
  code instead of repeating the `algorithmic_intervention_sufficiency` block.
  Solver evidence did not justify continuation. The first selector was
  abandoned with median `0.0`, CI `[-0.25, 0.0]`, win rate `0.125`, and CMT2
  median `-7.5`; v2 improved CMT2 to `+0.5` but stayed weak-positive below MDE
  with median `1.25`, CI `[-2.0, 6.0]` below the `9.9` MDE, CMT4 `-2.0`, and
...
25. Treat the CVRP-owned contract/context and route-pool closure repair as
   ...
   fork.
26. Treat successor51 `bounded_route_arc_lns_rebuild` as complete, valid, and
   active-marginal below MDE. It had direct phase telemetry and local positive
...
28. Treat successor54 `protected_budgeted_trajectory_selector` and
   `protected_budgeted_trajectory_selector_v2` as valid reviewed/default-avoid
   evidence for the protected race repair line. The gate repair succeeded, but
   the solver effect remained below MDE and CMT4/X-n110 stayed unsafe in v2.
