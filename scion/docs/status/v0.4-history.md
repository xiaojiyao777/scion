...
LLM-text, prompt-ratio, or raw cross-branch material. Required follow-up before
formal governance experiments: fixed-candidate or fixed-order replay controls,
clarified record-only/off ablation contract, manifest/source visibility
assertions, `measurement_governance` in `campaign_summary.json`,
`SCREENING_EXPAND_EXHAUSTED_BORDERLINE` review, fresh-runtime replay drain
closure, and durable parent-hypothesis lineage.
...
files, and `git diff --check`. The next formal governance experiment still needs
fixed-candidate or fixed-order replay controls and a precise record-only/off
ablation contract.
