...
guardrails, but no arm promoted, fresh-runtime replay pressure persisted, and
proposal trajectory session-to-formal-candidate joins need repair or
pre-registration before formal repeat analysis. CVRP remains excluded from
formal governance-value conclusions until measurement power improves.
...
- The warehouse proposal-context ablation shakedown validated prompt-visible
  control wiring, but proposal trajectory report joins are not yet robust
  enough for formal attribution: one arm had four replayable formal candidates
  and zero joined sessions. Campaign summaries also do not yet expose a
  top-level context-arm fingerprint. Fix these or pre-register the limitations
  before formal repeat analysis.
