...
## 2026-06-17 Warehouse Helper-Based Guard Static Gate Repair

The first local warehouse split/cost-runtime guidance rerun from commit
`bbb80db` was stopped as diagnostic evidence before field acceptance. The run
was at
`/home/clawd/research/scion-experiments/v04-warehouse-splitcost-runtime-rerun6r-bbb80db-20260617T080245Z`
and reached repeated pre-Protocol blocks (`8` proposal attempts, `8` quality
blocks, `0` protocol rows) before the main thread stopped it.

Trace inspection showed that this was not a context-loss failure: code prompts
contained the new warehouse guidance, prior quality-block feedback,
`repair_template`, and `missing_code_elements`. The
`warehouse_operator_module_delete` block was a valid contract block against
deleting a statically imported operator module. A separate
`merge_vehicles.py` attempt, however, used bounded candidate ranking and a
helper-based split/cost acceptance function, while the problem-owned static
guard still required a direct `if ...: return solution` shape. That made the
gate too narrow for a reasonable helper-based implementation.

The follow-up repair stays inside `WarehouseDeliveryAdapter`: it accepts
executable helper-based split/cost guards that reject non-improving candidates
with `return None` and no-op in the caller, while still rejecting
string/comment-only guard mentions. Focused validation passed warehouse target
preview (`37 passed`), agentic/proposal suites (`64 passed`),
telemetry/session preview suites (`48 passed`), py_compile, and
`git diff --check`. The next gate is a fresh local server `6R` rerun from the
follow-up commit.

That fresh rerun from commit `a1dba41` is complete and documented in
...
It is useful partial field evidence, not research-quality acceptance. Wrapper
`exit_code=0`; `run_validity.status=valid`; `reason=valid_partial_interrupted`;
`stopped_reason=proposal_attempt_limit_exhausted`; `effective_rounds_completed=5/6`;
`protocol_metric_results=10`; stage counts `screening=10`, `validation=0`,
`frozen=0`; `formal_screened_candidates=5`; `quality_blocks=13`; champion
stayed v1. The repair cleared the earlier zero-Protocol-row failure, but all
lineages still stayed screening-only and three lineages parked. The next task
is narrow code-stage behavior analysis for the blocked `move_order.py` and
`destroy_rebuild.py` patches before any WSL-scale warehouse matrix.

## 2026-06-17 Phase 4 Frontier After Warehouse Repair-Cap Gate

The latest Phase 4 frontier analysis is documented in
[`../experiments/v0.4/v04-phase4-frontier-analysis-20260617.md`](../experiments/v0.4/v04-phase4-frontier-analysis-20260617.md).

The warehouse repair-cap field gate from commit `f81bb73` completed as valid
framework evidence: `6/6` effective rounds, `7` protocol metric rows, stage
counts `screening=3`, `validation=2`, `frozen=1`, and no recurrence of
`telemetry_repair_attempt_budget_exhausted`. It is not promotion evidence:
champion stayed v1, and the strongest `merge_vehicles.py` candidate failed
frozen with median `-650.0`.

Branch-level analysis showed a specific research-quality failure shape rather
than a generic framework break. The candidate generated strong screening and
validation cost deltas, but nearly all `split_delta_sum` values were zero; the
frozen losses were total-cost regressions under preserved split count, with
runtime regression as well. The next repair is therefore warehouse
problem-owned guidance/quality: require split-positive versus
split-preserving cost-only effect scope, explicit frozen/runtime risk, and
bounded candidate enumeration before Protocol.

A local repair now updates `WarehouseDeliveryAdapter`, warehouse
`problem-v1.yaml`, and focused tests. It does not change generic Decision,
Protocol, validation/frozen, promotion, or `DecisionFeatures`. Main-thread
validation passed warehouse target preview (`34 passed`), agentic/proposal
suites (`64 passed`), telemetry/session preview suites (`48 passed`),
py_compile, and `git diff --check`. The next experiment gate is one local
server `6R` acceptance check.

The same frontier analysis also updates CVRP/VRP interpretation. CVRP is now
research-capable, with Phase C validation/frozen reach and branch depth, but it
is still not research-effective: no formal promotion, no canonical ALNS+VNS
improvement above MDE, and no robust broad mechanism from independent VRP
controls. The immediate next step is no-LLM family/slice mechanism diagnostics,
not another blind long LLM campaign.

## 2026-06-17 Warehouse Operator Diagnostics Follow-Up

The warehouse operator-diagnostics/patch-quality repair sequence reached a
clear next blocker. Commit `fdba51e` launched the short warehouse production
`6R` field gate at
`/home/clawd/research/scion-experiments/v04-warehouse-patch-quality-shape-rerun6r-fdba51e-20260617T052828Z`
with local `gpt-5.5`, production warehouse protocol/split/seeds,
`measurement_governance=on`, and `compact-measurement-diagnostics`.

The main thread intentionally stopped the run after failed-acceptance evidence
was sufficient. Wrapper `exit_code=1`; `run_validity.status=valid`,
`reason=valid_partial_interrupted`; `effective_rounds_completed=4`;
`protocol_metric_results=4`; `quality_blocks=6`; and no validation/frozen/
promotion row occurred. This is useful partial evidence, not a completed field
acceptance.

The result improved on the earlier `5c78f84` shakedown by clearing the
zero-protocol-row blocker. A newly created `same_subcategory_residual_merge`
operator produced consumed telemetry and continued same-mechanism exploration.
The field gate still failed because repeated warehouse patch-quality blocks
recurred, one hypothesis was blocked for missing validation-transfer risk, and
a positive `move_order.py` candidate was abandoned by telemetry mismatch:
declared mechanism `split_preserving_vehicle_elimination` did not match the
runtime-exported diagnostics key `move_order`. The next active repair is
warehouse-owned mechanism-identity/telemetry-key alignment for modified
existing operators, without changing Decision, Protocol, validation/frozen, or
promotion semantics.

That repair is locally accepted. `WarehouseDeliveryAdapter` now fail-closes
modify-existing warehouse operator hypotheses/patches before Protocol when
their declared telemetry mechanism ids do not match the runtime export key
derived from the existing operator module. The repair preserves the new-operator
path and leaves Decision, Protocol thresholds, validation/frozen gates, and
promotion semantics unchanged. Main-thread verification passed the focused
warehouse/proposal/agentic/telemetry regression suites and `git diff --check`.

## 2026-06-16 Warehouse Validation-Transfer Quality Repair

The warehouse positive-diagnostic rerun restored validation reachability but
still failed research-quality acceptance: the validation candidate failed
`VALIDATION_FAIL_NO_HIERARCHICAL_GAIN`, with no frozen row or promotion.

The follow-up code repair is locally accepted and documented in
...
It adds a generic optional adapter hook before code generation in both
non-agentic and agentic proposal paths. The hook itself is proposal-only; the
warehouse rule stays in `WarehouseDeliveryAdapter` and `problem-v1.yaml`.
Warehouse operator hypotheses must now explain screening-to-validation transfer
risk, a proposal-level activation/effect diagnostic plan, and a guard against
screening-only no-effect gains before code generation. The prompt guidance also
warns not to invent undeclared `expected_telemetry` keys for those counters.

No `DecisionFeatures`, Protocol thresholds, validation/frozen/promotion gates,
or validation/frozen per-case exposure rules changed. Acceptance passed
warehouse tests (`12 passed`), context/agentic tests (`90 passed`), proposal
pipeline tests (`64 passed`), Python compile, and `git diff --check`. The next
gate is a short warehouse production rerun from this repair commit; it should
field-check whether transfer-blind hypotheses are blocked before code or
whether screened candidates carry explicit transfer diagnostics before
validation is interpreted.

That short rerun was launched from commit `88e31d7` and early-stopped after it
exposed a narrower code-stage quality gap. Early-stop report:
...
The hypothesis-stage gate worked: there were `2` proposal quality blocks, both
recorded as `agent_quality_blocked` for missing transfer/diagnostic claims.
However, a non-blocked candidate reached screening with
`activation_status=not_declared` and `effect_status=not_declared`, even though
the hypothesis promised activation/effect counters. The field result therefore
rejects research-quality acceptance and opens the next repair: a problem-owned
patch/code quality hook that blocks warehouse operator patches when promised
validation-transfer diagnostics are not executable or observable before
Protocol.

That patch-quality repair is locally accepted and documented in
...
Core now provides only the generic `validate_problem_patch_quality(...)` hook
and lifecycle routing. `WarehouseDeliveryAdapter` owns the warehouse semantics
and blocks high-risk operator patches that lack code-visible activation/effect
diagnostics or a screening-only/lexicographic guard. The two early-stop field
trace patches are now both blocked before Protocol with
`agent_quality_blocked:warehouse_validation_transfer_patch_quality_missing`.
No `DecisionFeatures` or Protocol thresholds changed. Acceptance passed the
focused warehouse/proposal tests, broader proposal/context/session and
retry/evidence/cross-branch suites, Python compile, and `git diff --check`.
The next gate is a fresh short warehouse production rerun from the repair
commit.

The clean short rerun from commit `6e13b11` is complete at
`/home/clawd/research/scion-experiments/v04-warehouse-validation-transfer-patch-quality-rerun6r-6e13b11-20260616T203530Z`
after sync from WSL. It field-accepted the session-ref refresh but failed
research-quality acceptance: wrapper exit `0`, `6/6` effective screening rows,
`0` validation/frozen/promotions, and `6` proposal quality blocks. Four blocks
repeated `warehouse_validation_transfer_patch_quality_missing` for missing
activation/effect diagnostic code and, later, missing screening/lexicographic
guard. Code prompt manifests contained no prior quality-block feedback, so the
agent was not told in the code phase why the previous patch was blocked.

The follow-up local repair keeps the v3 boundary intact: prior quality blocks
are staged from a successful hypothesis into the immediately following code
context, rendered in the code prompt as proposal-only hard repair constraints,
and cleared after code success. Quality-block ledgers now preserve compact
session/ref fields when available so postrun analysis can link an accounting
row to the exact agentic session. No `DecisionFeatures`, Protocol thresholds,
or warehouse validation semantics changed. Acceptance passed the focused
proposal quality-block tests, broader proposal pipeline tests, warehouse
preview/agentic feedback tests, evidence status tests, Python compile, and
`git diff --check`. The next gate is another short warehouse field rerun from
this repair commit.

The `5f2d418` field rerun then accepted the visibility part of that repair in
prompt manifests and restored the full warehouse research path: wrapper exit
`0`, valid run, `6/6` effective rounds, `5` screening rows, `1` validation
row, `1` frozen row, and champion v2. Postrun:
...
It still exposed a second feedback-quality issue: generic agentic rejection
projection could overwrite the problem-owned warehouse retry constraint with
generic novelty text. The follow-up repair preserves the structured retry
constraint when present and carries `missing_claims` / `missing_code_elements`
forward into quality feedback, keeping actionable warehouse patch requirements
visible without adding warehouse semantics to generic Decision. A fresh
`3c2b7b5` field rerun then accepted that narrower repair but failed research
quality. The run was valid and complete (`6/6` effective rounds) and preserved
warehouse-owned retry constraints in quality-block ledgers and code prompt
traces. It still produced only screening rows (`6` screening, `0` validation,
`0` frozen, no promotion), and `9/15` proposal attempts were quality-blocked.
This moved the blocker from observability/constraint propagation to hypothesis
quality: prior blocks must be hard constraints before the next research
hypothesis is proposed, not only before code is generated.

The follow-up commit `4b2ee29` added that hypothesis prompt section. Its fresh
warehouse short field check accepted prompt visibility but still failed
research quality. Postrun:
...
The run was valid and complete (`6/6` effective rounds), and `7/12`
hypothesis traces included `Prior Agent Quality Blocks For This Hypothesis`.
It still produced only screening rows (`6` screening, `0` validation, `0`
frozen, no promotion), with `5/11` proposal attempts quality-blocked. The next
repair adds warehouse problem-owned `repair_template` checklists so visible
quality blocks tell the agent exactly which hypothesis fields, diagnostic
identifiers, and screening/lexicographic guard code shape must be repaired.
The `4a316e1` repair-template field check completed but was invalid as
warehouse research evidence. Launch report:
...
The wrapper exited `0`, but `run_validity.status=invalid`,
`reason=invalid_no_protocol_rows`, with `0` protocol rows. The root cause was a
copied-split data-root mismatch: relative `safe_data_roots` resolved to
`/home/clawd/scion-data` while production cases live under
`/home/clawd/research/scion-data`.

The follow-up data-root repair was accepted by a server-side `6R` rerun from
preflight commit `ad469f0`. Launch report:
...
The run was valid and complete (`6/6` effective rounds,
`formal_screened_candidates=6`, `protocol_metric_results=6`, no verification
failures), so the copied-config data-root path is no longer the blocker.
Research quality still failed: all formal rows remained screening-only, with
`0` validation/frozen rows and no promotion. The retained `ed329b23` and
`792e6d6e` branches now require branch/prompt/artifact analysis before another
warehouse rerun.

Boole's branch-level analysis identified a narrower telemetry wiring defect:
accepted warehouse operators were writing `validation_transfer_diagnostics`
dictionaries, but formal metrics still exposed activation/effect diagnostics as
`not_declared`. The follow-up operator diagnostics telemetry repair is locally
accepted and documented in
...
The surrogate solver now exports operator instance
`self.validation_transfer_diagnostics` as
`operator_diagnostics.{mechanism}.*`; both warehouse `problem-v1.yaml` copies
declare those telemetry fields; and warehouse patch-quality validation now
fails closed when a patch only keeps local diagnostics dictionaries that the
solver cannot export. This did not change Decision/Protocol promotion gates and
did not add LLM text, prompt context, or branch lessons to `DecisionFeatures`.
The next gate is a short warehouse production `6R` check from the repair
commit, with acceptance based on declared operator diagnostics being consumed
instead of `not_declared` telemetry.

Two short field-gate shakedowns followed. The first, from commit `9ad5465`,
was stopped before protocol rows after repeated `schema_retry_drift`: schema
repair treated bare structural counter names such as `operator_invocations` as
mechanism ids. The local follow-up commit `5c78f84` fixed that problem by
declaring warehouse structural activation refs and preserving nested runtime
map mechanism identity. Its focused tests passed, but the follow-up server
`6R` shakedown at
`/home/clawd/research/scion-experiments/v04-warehouse-operator-diagnostics-telemetry-rerun6r-5c78f84-20260617T050235Z`
was also stopped as invalid before protocol rows:
`run_validity.status=invalid`, `reason=invalid_no_effective_rounds`,
`protocol_metric_results=0`, and `quality_blocks=2`. Launch/postrun report:
...
This accepts the schema retry repair only as clearing the prior blocker; it is
not warehouse research evidence. The new blocker is warehouse problem-owned
patch-quality precision: first code prompts need the accepted diagnostics/guard
scaffold, and the static check must accept valid helper/alias diagnostics plus
split/cost guard shapes while still failing closed on comments, local-only
dictionaries, and missing guards.

The local patch-quality shape repair is accepted. Warehouse code prompts now
receive active-subject code constraints for exportable validation-transfer
diagnostics and executable guards, and the static quality gate accepts
helper-returned standard diagnostics dictionaries assigned to
`self.validation_transfer_diagnostics`, alias mutations, split/cost delta
guards, and candidate/base split-cost guards. It still rejects comments,
strings, local-only dictionaries, and missing keys. Focused verification passed
warehouse preview (`26 passed`), provider/proposal quality (`43 passed`),
telemetry/expected-telemetry (`41 passed`), schema retry (`24 passed`),
py_compile, and `git diff --check`. The next warehouse gate should be one
short production `6R` rerun from the repair commit.

## 2026-06-16 Warehouse Lifecycle Rerun Still Screening-Only

The warehouse loss-heavy lifecycle rerun from commit `6e3988c` is complete and
documented in
...
It ran as a single `6R` production warehouse cell on WSL, exited `0`, completed
`6/6` effective rounds, and had no proposal quality blocks, verification
failures, or fresh-runtime replay drain.

The prior loss-dominated marginal loop did not recur, so the lifecycle brake is
field no-regression rather than direct field-trigger proof. Positive marginal
branch depth was preserved. Research quality remains rejected: every protocol
row stayed in screening and no validation, frozen row, or promotion occurred.
The next specific blocker is warehouse-owned protocol admission for
expanded-exhausted positive low-SNR evidence. The strongest shape was
`3/1/10` case W/L/T, `13/6/9` pair W/L/T, median `300`, and CI low `0`, which
continued exploration rather than queueing diagnostic validation.

That warehouse-owned protocol repair is now implemented and accepted
...
Warehouse production diagnostic thresholds were relaxed to
`pair_win_rate_min=0.46` and `pair_non_tie_win_rate_min=0.68`. Generic Decision
code did not change. Direct replay now queues the field-positive shape for
diagnostic validation while negative-median and loss-dominated shapes still do
not validate.

The follow-up field gate from commit `41d02d1` is complete with split
acceptance. Postrun:
...
It was valid, completed `6/6` effective rounds, and produced `5` screening rows
plus `1` validation row. This field-accepts validation reachability and the
loss-heavy lifecycle brake: the repeated `move_order.py` `1/2/3` case,
`3/4/5` pair shape was parked. Research quality remains rejected because the
validation candidate failed `VALIDATION_FAIL_NO_HIERARCHICAL_GAIN`, with no
frozen row or promotion.

## 2026-06-16 Warehouse Pair-Signal Rerun Failed Research Quality

The warehouse pair-signal diagnostic rerun from commit `3ca26a0` is complete and
documented in
...
It ran as a single `6R` production warehouse cell on WSL, finished with wrapper
exit `0`, completed `6/6` effective rounds, and produced no proposal quality
blocks, verification failures, or fresh-runtime replay drain.

The pair-signal protocol repair is accepted as fail-closed. No candidate met the
new conservative diagnostic-validation path: the expanded-exhausted
`consolidate_subcategory` candidate became negative-median and pair-loss-heavy,
and the deeper `move_order.py` branch repeated `1/2/3` case W/L/T, `3/4/5`
pair W/L/T, and median `0.0` four times. Research quality is therefore still
rejected: no validation, no frozen row, no promotion, and no measurable
same-branch behavior progress.

The accepted next repair is a generic lifecycle brake for loss-dominated
marginal/no-effect follow-ups. `BranchLifecyclePolicy` now parks those follow-ups
when case losses exceed wins, pair losses are at least pair wins, and median
delta is non-positive, while preserving pair-positive marginal branch depth.
This remains v3-aligned: it consumes only deterministic `DecisionFeatures`
numbers plus generic lifecycle tiers and adds no warehouse/CVRP semantics to
generic core.

## 2026-06-16 Warehouse Rerun Accepted Only For Blocker Repair

The warehouse research-loop repair rerun from commit `d666311` is complete and
documented in
...
It ran as a single `6R` production warehouse cell on WSL with local `gpt-5.5`,
`measurement_governance=on`, compact measurement diagnostics, disabled early
stop, and a `30s` solver cap.

The repair slice is field-accepted: wrapper exit was `0`,
`run_validity.status=valid`, `6/6` effective rounds completed, all six formal
candidates reconciled, proposal quality blocks fell to `0`, verification-
consumed failures fell to `0`, and no fresh-runtime replay row drained the run.
Compared with the failed `0bb99ec` gate, branch-lesson semantic repair and
warehouse operator preview both worked in the field.

Research quality is not accepted. All six protocol rows remained screening
only, with no validation, frozen, promotion, or restored v0.3-style continuous
promotion. The deepest branch was a same-mechanism `move_order.py`
`fill_slack_order_move` sequence with pair-level positive but case-level
borderline evidence (`2/0/4` case W/L/T, `6/2/4` pair W/L/T, median `0.0`,
CI crossing zero). The next repair should target warehouse measurement and
production-protocol semantics: declare the low-SNR/pair-level behavior in the
problem-owned layer and allow conservative diagnostic validation for
pair-positive, non-regressive borderline signals. This must not loosen
validation/frozen/promotion gates or expose raw LLM/prompt material to
`DecisionFeatures`.

The measurement/protocol repair is now implemented and accepted as
deterministic behavior only. Report:
...
Warehouse measurement now declares `pairing_validity=trajectory_divergent`, and
`protocol_prod.yaml` enables conservative pair-level diagnostic validation
after expanded screening is exhausted. Focused acceptance passed config,
problem-bridge, decision-screening, and protocol-gate tests (`97 passed`) plus
a direct deterministic replay of the rerun shape. This is not yet warehouse
efficacy evidence; it requires a short production rerun to see whether
pair-positive branches reach validation and are then confirmed or rejected.

## 2026-06-16 Warehouse Research-Loop Quality Gate Failed

The warehouse research-loop repair short `6R` field check is complete and
documented in
...
It ran from commit `0bb99ec` with local `gpt-5.5`, production warehouse
protocol/split/seeds, `measurement_governance=on`, and
`compact-measurement-diagnostics`. Execution was valid and complete (`6/6`
effective rounds, `7` proposal attempts, `4` screening rows), but the
research-quality gate failed: no validation/frozen/promotion occurred, one
`branch_lesson_usage_semantic_mismatch` block remained, two candidates consumed
verification failures, and same-branch follow-up did not produce useful
validation evidence.

The accepted next repair is targeted rather than another observability pass:
canonical proposal-only repair for concrete `branch_lesson_usage` values that
already carry a specific lesson signal, plus a warehouse problem-owned operator
preview that rejects existing imported operator deletion and catches undeclared
local nested dict state-key references before V5. Protocol, DecisionFeatures,
promotion, and gate semantics remain unchanged. This repair is code accepted
only; the next warehouse live field check must prove behavioral improvement.
...
A tenth independent VRP-only control later completed as subagent `Ohm`
(`019ecd40-b8cd-7060-85e0-dc28394e2cb7`) under
`/home/clawd/research/vrp-independent-codex-research/phase-i-20260615` and is
documented in
...
It found `rotated_sweep8`, a large construction-fallback hypothesis that tries
up to `8` angular sweep starts. On `10` AGS seed-0 pairs the candidate produced
W/T/L `7/3/0`, total-distance delta `-61,847`, and mean BKS-gap delta
`-1.184` pp. This is external-control hypothesis material only; it improves a
weak construction fallback but does not address the main X-subset ALNS gap.

An eleventh independent VRP-only control was then launched as subagent
`Russell` (`019ecd64-52d1-75b3-8b39-45ca1a78b7eb`) under
`/home/clawd/research/vrp-independent-codex-research/phase-j-20260615` and is
documented in
...
It is a fresh, non-forked external research subject and must not read Scion
design, task, status, audit, planning, prompt, or experiment artifacts. Its job
is to keep the plain-Codex VRP baseline research lane active with a detailed
research-process trace. Any positive candidate is external hypothesis material
only and must pass later no-LLM replay before it can influence Scion
experiments or default solver settings.

Russell then completed with a negative result. Result report:
...
contains the required process files, `4` valid JSONL experiment rows, and no
retained `candidate.patch`. It screened three bounded candidates on hard
X-focused paired cases: `no_vns_param` was rejected with W/T/L `0/0/15` and
mean cost delta `+240.73`; `rotated_sweep_initial` and
`max_destroy_160_param` were neutral at W/T/L `0/15/0`. This is negative
external-control evidence rather than a Scion replay seed.

A read-only CVRP behavior audit then completed as subagent `Gibbs`
(`019ecd49-1c1b-7240-afa1-57084260772c`) and is documented in
...
It judged Scion CVRP as research-capable but not yet research-effective: Phase C
has real branch depth, branch lesson fields, source visibility, and
validation/frozen reach, but no promotion, canonical ALNS+VNS stayed below MDE,
large-X positives collapsed, and lesson usage is not yet causally tied to
better mechanism choices.
...
`39 passed`. Launch was intentionally deferred until the CVRP size70 large-X
solver load cleared, so the warehouse `30s` lifecycle debug would not be
distorted by parallel CVRP solver jobs.

The CVRP size70 large-X diagnostic replay then completed and synced back from
WSL. Launch report:
[`../experiments/v0.4/v04-cvrp-twoopt-size70-largeX-launch-20260615.md`](../experiments/v0.4/v04-cvrp-twoopt-size70-largeX-launch-20260615.md).
Postrun:
...
Server root:
`/home/clawd/research/scion-experiments/v04-cvrp-twoopt-polish-followup-smoke-20260615T1848Z`.
The replay planned `24` candidate rows (`4` large-X cases x `3` seeds x
multipliers `{1,4}`), produced `24` candidate rows, and completed `23`
candidate/champion pairs. The only planned noncompleted pair was
`X-n1001-k43 seed61 m1`, where both baseline and candidate timed out. Completed
pairs were all wins: W/L/T `23/0/0`, mean candidate-minus-champion delta
`-295.5652`, median `-192.0`, route/fleet regressions `0`, initial two-opt
accepts `23`, embedded accepts `72`, and `candidate_best_update_count=0`. This
satisfies the no-LLM large-X objective-movement diagnostic gate. It does not
support promotion by itself because it is direct replay, runtime pressure is
heavy, `m=2` was intentionally not run, and the gain comes through
construction/two-opt polish rather than ALNS incumbent-update trace. The
warehouse 1-candidate lifecycle debug could launch because the WSL CVRP solver
load had cleared.

A sixth independent VRP-only external control was launched as subagent
`Lovelace` (`019ecce4-4806-7581-b033-d33911f8b276`). It is a fresh,
non-forked plain Codex researcher forbidden from reading `scion/`, `TASK.md`,
Scion design docs, Scion audit reports, Scion status docs, and Scion experiment
artifacts. Launch report:
...
The purpose is long-running standalone `vrp/` baseline research with a full
process log. Any result remains external-control process evidence and may only
seed later no-LLM Scion replay.

The warehouse 1-candidate lifecycle debug then completed. Postrun:
...
`protocol_metric_results=0`, `formal_screened_candidates=0`, and canary failed
before formal Protocol because the copied split manifest resolved
`safe_data_roots` to the wrong WSL path. Accepted rerun root:
`/home/clawd/research/scion-experiments/v04-warehouse-1candidate-lifecycle-debug-20260615T2000Z`.
The rerun rewrote `safe_data_roots` to
`/home/xjy-ubuntu/research/scion-data`, exited `0`, and passed the lifecycle
gate with `run_validity.status=valid`, `effective_rounds_completed=1`,
...
no postrun failures. The screened candidate
`operators/subcategory_consolidate_upgrade.py` reached Decision
`continue_explore` with reason codes `SCREENING_FAIL_WIN_RATE` and
`SCREENING_MARGINAL_SIGNAL_CONTINUE`; screening case W/L/T was `1/2/7`, pair
W/L/T was `5/8/7`, and median delta was `0.0`. This proves the repaired
warehouse path can reach Protocol again, but it is not promotion evidence.
Next step: short `3-5R` compact warehouse debug before a full `3 x 24R`
longrun rerun.

That short warehouse debug was launched as a `3R` compact ON-arm run. Launch
report:
...
`2026-06-15T20:14:17Z` from commit `dabfcee`, with local `gpt-5.5`,
`measurement_governance=on`, `compact-measurement-diagnostics`, `rounds=3`,
`time_limit_sec=30`, disabled early stop, and the corrected absolute WSL safe
root. Initial health check confirmed campaign startup and the expected status
and database artifacts.

The short warehouse debug then completed and was postrun-audited. Postrun:
...
`effective_rounds_completed=3`, `protocol_metric_results=2`, and
`formal_screened_candidates=2`. The third attempt consumed the remaining budget
but failed before Protocol on `V9_perf_guard` heavy runtime (`1421ms` candidate
versus `445ms` champion on `instance_small_1.json`). The first warehouse
candidate created `operators/subcategory_pack_upgrade.py` and was marginal
(`1/2/7` case W/L/T, `5/8/7` pair W/L/T); the second modified the same file on
the same branch and became no-effect (`0/0/6`, `0/0/12`); the third clean-fork
`operators/swap_orders.py` candidate failed Verification. Branch lesson usage
was present in all six sessions, but prompt payloads remained large and
tool-selection/general heavy. A read-only semantic audit judged that later
prompts did receive branch lessons, but only the same-branch modify clearly
used them well; the clean-fork `swap_orders.py` candidate still failed runtime
verification. Interpretation: framework path and branch continuation work, but
warehouse continuous improvement is not recovered yet.

The sixth independent VRP-only external control `Lovelace` also completed.
Result report:
[`../experiments/v0.4/v04-independent-vrp-research-agent-20260615e.md`](../experiments/v0.4/v04-independent-vrp-research-agent-20260615e.md).
It retained a cleanly applying `construction_portfolio` `candidate.patch`, with
`1s` result `2` improved / `0` worse / `24` same and `2s` expansion
`2` improved / `2` worse / `21` same. Caveat: the observed baseline included
the pre-existing dirty standalone `vrp/src/solver.py`, so this is an external
backlog seed, not clean Scion evidence.

The CVRP size70 fixed-candidate validation design was completed. Report:
...
The next CVRP order is fixed-candidate replay first, then seeded Scion CVRP run
only after mechanism validity passes. The large-X completion diagnostic must
include `m=2` and the current tool gap is stage-aware validation/frozen replay
for an external full-file candidate artifact.

That CVRP size70 Tier 1 Large-X completion diagnostic later completed and was
accepted. Launch report:
...
It accounted all `36/36` planned keys across `X-n401`, `X-n573`, `X-n641`,
`X-n1001`, seeds `61/67/89`, and multipliers `1/2/4`. Candidate had
`35 completed / 1 timeout`; champion curve had `34 completed / 2 timeout`; no
timeout/failure was candidate-only. On `34` completed-vs-completed pairs,
candidate won `34/0/0` with total-distance sum delta `-10014.0`, stable route
counts, and no fleet violation. `two_opt_polish_initial` improved `35/35`
completed rows and `two_opt_polish_embedded` improved `26/35`. The postrun
passes Tier 1 to formal fixed-candidate validation readiness, but the evidence
remains no-LLM mechanism-validity material only. `X-n1001` remains a heavy-tail
runtime diagnostic, and `best_update_count=0` means the mechanism claim is
limited to two-opt polish phase movement rather than deeper ALNS incumbent
updates.

The CVRP size70 fixed-candidate validation replay was then launched on WSL.
Launch report:
...
Initial tmux session: `scion_cvrp_size70_validation_223636`. The WSL repo was updated
to commit `2e0db05`; the external candidate artifact and validation manifest
were prepared with WSL-local paths; and the manifest materialization check
recreated the expected size70 `scheduler.py` file. The replay runs one external
full-file candidate over both fixed replay arms at validation stage with formal
CVRP protocol/split/seed configuration. It is still no-LLM/no-APS
mechanism-validity replay, not Scion promotion evidence.
The first attempt failed before Protocol with `error_count=2` because the
formal split manifest lacked a WSL `safe_data_roots` entry, so strict case-path
resolution could not resolve `cvrplib/A/A-n60-k9.vrp`. A repaired replay was
launched at
...
experiment-local split manifest adding `/home/xjy-ubuntu/research/or-autoresearch-agent/vrp`.
That repaired replay was then stopped as invalid-spec shakedown: raw metrics
showed `total_pairs=32` because formal `protocol.yaml` uses
`validation.n_cases=8`, while the pre-registered Tier 2 requires all
`12` validation cases x `4` seeds = `48` pairs per arm. A full validation
relaunch is active at
`/home/xjy-ubuntu/research/scion-experiments/v04-cvrp-size70-fixed-validation-full-20260615T225148Z`
with tmux session `scion_cvrp_size70_validation_full_225148`, an
experiment-local protocol override `validation.n_cases=12` /
`validation.expand_to=12`, and WSL `safe_data_roots`; initial raw metrics report
`total_pairs=48`. A later sync found the full replay still active, with no
`exit_code.txt`, no top-level comparison yet, and raw validation metrics at
`9/48` attempted/valid pairs, `0` failed pairs, W/T/L `0/8/1`, mean delta
`-0.222`, and median delta `0.0`. This is progress telemetry only; do not
accept or reject the mechanism until all `48` pairs and comparison outputs are
present. A later sync clarified that the `48` planned pairs are per replay
arm: the `on` arm completed `48/48` with `0` failed pairs, and the run then
started the `record_only` arm. This remains an active run until `record_only`
finishes, `exit_code.txt` is written, and the top-level comparison artifact is
available.

The twelfth independent VRP-only external control `Helmholtz`
(`019ecd83-3324-7820-912e-2c7d94517e7e`) was launched as a fresh, non-forked
plain Codex researcher. Artifact root:
`/home/clawd/research/vrp-independent-codex-research/phase-k-20260615`.
It is intentionally not a Scion subagent and is forbidden from reading
`scion/`, `TASK.md`, Scion design/status/audit/planning/experiment artifacts,
or prior Scion conclusions. It may only study standalone `vrp/`, record its
research process in `research_log.md`, write experiment rows to
`experiments.jsonl`, summarize in `summary.md`, and optionally retain a
`candidate.patch`. The purpose is external-control process evidence: whether an
uncontaminated plain Codex VRP researcher can improve the same baseline. Any
positive result is only a hypothesis seed until separate no-LLM replay validates
it.

`Helmholtz` then completed. Result report:
...
Artifact root:
`/home/clawd/research/vrp-independent-codex-research/phase-k-20260615`.
The required process artifacts are present, `experiments.jsonl` parses with
`3` rows, and `candidate.patch` dry-runs against a temporary clean `HEAD`
archive. The retained H2 `regret4_repair` patch adds a `regret4` ALNS repair
operator and measured W/T/L `8/5/2`, mean delta `-32.333`, median delta
`-11.0`, failures `0` on `5` cases x `3` seeds at `1s`. Treat it as a positive
external-control hypothesis seed only: it has two X-family regressions and
requires broader no-LLM validation before any Scion replay or default solver
change.

The CVRP size70 fixed-candidate validation replay then completed and was
accepted as valid fixed-candidate evidence. Postrun:
...
The wrapper exited `0`, comparison schema was
`scion.fixed_candidate_replay_comparison.v1`, `row_count=2`, `candidate_count=1`,
and both `on` and `record_only` rows completed with no row errors and no
campaign/promotion/scheduler mutation. Raw metrics completed both arms at
`48/48` valid pairs with `0` failed pairs. ON W/T/L was `26/13/9`, mean delta
`34.4167`, median delta `4.0`; `record_only` W/T/L was `27/13/8`, mean delta
`34.5208`, median delta `6.0`. Both arms nevertheless reported
`runtime_gate_visibility.gate_outcome=fail` with
`VALIDATION_FAIL_NO_HIERARCHICAL_GAIN`. Runtime evidence was sufficient/high,
route deltas were all `0`, fleet violation was always `0`, and two-opt
activation was visible. Decision: stop the size70 candidate at validation; do
not launch frozen fixed replay and do not seed a Scion CVRP campaign from this
candidate as a validated mechanism.

The thirteenth independent VRP-only external control `Newton`
(`019ecded-8a24-7a03-b64b-8b1929c9af49`) was launched as a fresh, non-forked
plain Codex researcher. Launch report:
...
It is intentionally not a Scion subagent and is forbidden from reading
`scion/`, `TASK.md`, Scion design/status/audit/planning/experiment artifacts,
or prior Scion conclusions. It may only study standalone `vrp/`, available VRP
problem data, and standard Python tooling. Required artifacts are
`research_journal.md`, `candidate.patch` or `rejected_candidates.md`,
machine-readable experiment results, `summary.md`, and `README.md`. The first
cycle is bounded to real cases, at least `3` seeds, and a `1s` initial budget.
Its output is external-control process evidence and hypothesis material only;
any positive candidate still requires broader no-LLM validation before Scion
replay, Protocol evidence, or default solver changes.

`Newton` then completed. Result report:
...
The required process artifacts are present under
`/home/clawd/research/vrp-independent-codex-research/phase-l-20260616`, and no
`candidate.patch` was retained. The implemented intra-route Or-opt VNS
candidate was rejected after `30` paired real CVRPLIB runs at `1s` and `3`
seeds. Combined W/T/L was `7/12/11`, mean delta `+1.6`, median delta `0.0`,
with `0` subprocess errors, `0` CVRP feasibility failures, and `0` candidate
route-count regressions. The outside-case sanity slice was negative at W/T/L
`2/1/6`, mean delta `+5.666667`, and median delta `+6.0`. Treat this as
negative external-control evidence; it should not seed broader replay, Scion
fixed replay, or solver adoption.

A new warehouse longrun regression check against the v0.3 promotion reference
was launched. Launch report:
...
It uses commit `f384884`, `measurement_governance=on`,
`compact-measurement-diagnostics`, production warehouse protocol/split/seeds,
`3` repeats x `24R`, `30s` solver cap, disabled early stop, local `gpt-5.5`,
and max WSL parallelism `2`. The postrun acceptance must compare promotion
cadence and final champion version with champion quality/gap, Protocol row
completeness, branch depth, branch-lesson semantic usage, prompt composition,
and whether any no-promotion result is real plateau or framework failure.
WSL tmux session `scion_wh_longrun_reg_071323` started successfully at
`2026-06-16T07:16:00Z`; the initial health check showed `status=running`,
commit `f384884`, and `rep01` started. Monitor/postrun worker `Sartre`
(`019ecf4a-359e-7870-8764-ae389aafb106`) was launched to poll sparingly,
sync the WSL root when complete, and write the postrun report.

The CVRP/VRP `regret4_repair` broader no-LLM validation was delegated to
Scion worker `Aquinas` (`019ecf46-fbac-7a42-b360-b1bd4aecf6a0`). It must read
v3 first, use clean baseline/candidate scratch workspaces from `git archive
HEAD`, apply the phase K `candidate.patch` only in the candidate copy, avoid
LLM/APS calls, and write artifacts under
`/home/clawd/research/scion-experiments/v04-vrp-regret4-broader-validation-20260616`.

The warehouse longrun regression check then completed and was accepted as
valid longrun evidence. Postrun:
...
Server root:
`/home/clawd/research/scion-experiments/v04-warehouse-longrun-regression-3x24r-20260616T071323Z`.
The wrapper exited `0`, all three cells exited `0`, and all repeats reached
Protocol, validation, and frozen. Promotions were `rep01=1`, `rep02=0`,
`rep03=1`; final champions were `v2`, `v1`, and `v2`. The run rules out a
catastrophic v0.4 warehouse launch/preflight regression and proves warehouse can
still promote under the current v0.4 path. It does not recover the v0.3 cadence:
v0.3 production Sonnet promoted `3/3` after fixes and the strongest synthetic
run showed a `4`-promotion chain, while this v0.4 run produced `2/3` single
promotions. The postrun identifies proposal/context/code-edit quality plus
unresolved fresh-runtime replay closure as the limiting path, not lack of
warehouse opportunity.

The CVRP/VRP `regret4_repair` broader no-LLM validation then completed.
Postrun:
...
Artifact root:
`/home/clawd/research/scion-experiments/v04-vrp-regret4-broader-validation-20260616`.
All `80/80` rows completed using clean archive-based baseline/candidate
workspaces. Overall W/T/L was `21/31/28`, W-L `-7`, mean delta `-4.4625`,
median delta `0.0`, failures `0`, feasibility regressions `0`, and route-count
regressions `0`; repeated regression families were `E`, `M`, and `P`. Decision:
reject as-is for Scion fixed replay. If revisited, it should be a narrower
X-slice diagnostic or a new mechanism-family hypothesis.

Warehouse longrun per-cell branch/process analysis workers were launched:
`Kepler` (`019ed0da-651c-7741-99be-bc770758840b`) for `rep01`, `Russell`
(`019ed0da-a9a6-7b63-b4f4-eadecf7ad988`) for `rep02`, and `Curie`
(`019ed0da-ed19-7393-b91e-beee247e2624`) for `rep03`. Each has a v3-first
brief and must analyze branch evolution, per-round context/output,
branch-lesson transfer, quality failures, and measurement/noise before writing
its report. VRP independent-research process audit worker `Halley`
(`019ed0db-448e-76a0-865c-901917310c3c`) was also launched to inspect the
independent process logs and explain why residual VRP BKS gap has not yet
translated into a robust broad candidate.

All four process audits then completed and were accepted as diagnostic inputs:
...
The accepted interpretation is that v0.4 warehouse is not catastrophically
regressed: the `3 x 24R` longrun reached Protocol, validation, and frozen in all
repeats and promoted in `2/3` repeats. It still does not recover the v0.3
cadence or continuous branch-improvement behavior. The branch-level bottleneck
is research-loop efficiency: branch lessons are visible and semantically
projected in every inspected warehouse session, but many traces truncate
branch-lesson context and proposal attempts often fail strict target/action/
mechanism linkage. The same replay also shows fragile same-file edit handling,
cheap verification misses, all-tie weak-positive continuations, and
fresh-runtime replay pressure that could not schedule a replay candidate.

The VRP independent audit answered the BKS-gap concern separately. BKS gap is
real headroom, but it is not a paired-improvement certificate. Independent
agents kept usable process records and bounded matrices, but the retained
`regret4_repair` smoke signal failed broader validation (`21/31/28`, W-L `-7`,
repeated regressions in `E`, `M`, and `P`), and the phase-L Or-opt VNS candidate
was rejected. The next VRP task should be mechanism diagnosis by family/slice
and runtime phase, not another broad smoke-first local-operator search.

After reviewing the synthesis against the v3 blueprint, the 6/11 framework
audits, and the evidence-uplift roadmap, a new blocking repair was opened:
W4/P2 prompt signal density. The warehouse longrun reports showed that
`compact_research_signals` and `branch_lesson_usage_context` still carried
provider-visible truncation markers in many hypothesis traces. Manual inspection
confirmed the root cause is Scion-side prompt rendering: critical research
sections are still routed through `_bounded_json` budgets, and branch-lesson
`required_response`/compliance payload is still repeated in proposal input.
This is a v0.4 closeout blocker, not a v0.5 experiment item.

Acceptance plan:
[`../planning/v0.4/v04-prompt-signal-density-repair-acceptance-20260616.md`](../planning/v0.4/v04-prompt-signal-density-repair-acceptance-20260616.md).
Concrete development is delegated to worker `Lovelace`
(`019ed10e-aa46-7e02-bcdc-71d5d05383b4`). Main-session acceptance must prove
that compact research signals, branch lesson usage context, and cross-branch
research map are not prompt-section truncated under a stress context, while
full artifacts and v3 Decision boundaries remain intact.

The W4/P2 prompt signal-density repair is now implemented and accepted.
`compact_research_signals` now acts as a short index rather than embedding the
full cross-branch payload; `branch_lesson_usage_context` keeps compact lesson
ids, required/candidate linkage, target/action/mechanism fields, maturity, and
evidence summaries without repeating per-lesson `required_response` templates;
and `cross_branch_research_map` renders mechanism-level signals rather than
full portfolio/audit/session payloads. Main-session verification passed:
focused prompt/validation/branch-lesson suite (`79 passed`), proposal
pipeline/artifact subset (`45 passed`), Python compile for touched prompt
modules, and `git diff --check`. This accepts the code repair; the next live
warehouse/CVRP campaign should still check prompt manifests to confirm no
critical research sections are truncated in real traces.

User review then clarified the v0.4 research-stage policy: do not optimize
prompt rendering around hard budgets, fixed item caps, or field-level
truncation yet. The model context is sufficient for the current debugging
phase; Scion should first ensure effective research signal is present and only
remove noise/raw audit/compliance payloads. Worker `Kant`
(`019ed11a-7885-7b40-840f-20ba66678ee9`) implemented commit `fd185cf`
(`fix: remove prompt research signal hard caps`) to remove the remaining
default character caps, list caps, array slicing, and ellipsis/truncation
markers from projected research signals. Main-session acceptance reran the
combined prompt/validation/branch-lesson plus proposal pipeline/artifact suite
(`124 passed`), Python compile, and `git diff --check`. The next live campaign
should validate both signal completeness and noise exclusion in real prompt
manifests.

After that acceptance, the main session launched a warehouse no-hard-truncation
short `4R` field check. Launch report:
...
It runs commit `061eba0` with local `gpt-5.5`, production warehouse protocol,
`measurement_governance=on`, `compact-measurement-diagnostics`, disabled early
stop, and `4` rounds. The run is a prompt-manifest field check, not a longrun
research-efficiency claim. A first root ending `155433Z` was marked invalid
launch-env evidence only because `SCION_API_KEY` was not passed to the Scion
LLM client and APS failed with missing credentials.

The official `155951Z` field check completed cleanly and is accepted only for
prompt/context validity. Postrun:
...
It finished with wrapper `exit_code=0`, `run_validity.status=valid`, `4/4`
effective rounds, and `21` LLM traces all on `gpt-5.5`. The required
hypothesis research sections were included and non-truncated, and the prompt
checks found no raw audit/session/`required_response` noise or synthetic
research-signal truncation markers. Research quality still failed: there were
only `3` screening rows, all failed by win rate, with no validation/frozen rows
and no promotion. The next repair should therefore move from prompt rendering
to warehouse research-loop quality: semantic branch-lesson feedback,
solution-consistency prevention, and structured same-branch follow-up for the
surviving marginal residual-vehicle absorption signal.

Worker `Huygens` (`019ed14e-6257-7573-9000-29a2b4fb145d`) then implemented the
first warehouse research-loop quality repair slice, accepted as code but not as
an efficacy claim. Branch-lesson repair skeletons now emit deterministic
`required_item_fields`, `expected_linkage`, and `corrected_fields` for the
lesson id, target/action linkage, mechanism id/family, and changed dimensions
needed to repair semantic mismatches. Hypothesis prompts now add marginal
same-branch refinement guidance when branch hygiene exposes an
`active_marginal`/`marginal` branch, including visible positive cases when
available plus required causal `effect_path` and `no_op_condition` guard
language. Warehouse `vehicle_level` surface guidance now warns against unsafe
split/merge operator deletion or replacement without an explicit assignment
completeness and `Solution.assignment`/`Vehicle.order_ids` synchronization
plan. Main-session verification passed focused branch-lesson, branch-prompt,
and warehouse preview tests (`38 passed`), broader proposal suites
(`98 passed`), branch lifecycle and research-surface suites (`110 passed`),
Python compile, and `git diff --check`. No DecisionFeatures, Protocol,
promotion, or gate semantics changed. Residual risk: unsafe split/merge removal
is still prompt/spec-guided rather than a deterministic operator-surface
pre-code blocker. The next live gate should be a short warehouse field check
focused on research quality, not prompt visibility.

The main session launched that short warehouse field check on WSL as
`v04-warehouse-research-loop-repair-short-6r-20260616T165145Z`. Launch report:
...
Initial health passed: WSL preflight validated the checkout, Python, model
proxy, data roots, and surrogate root; the run entered `status=running` and
`run.log` reached warehouse campaign startup. Acceptance should require valid
wrapper/campaign artifacts and improved research-loop behavior, especially
fewer or repaired semantic branch-lesson quality blocks, no repeat unsafe
split/merge-removal `V5_solution_consistency` failure, and same-mechanism
causal follow-up for any retained marginal branch.

On 2026-06-16, warehouse validation-transfer patch-quality field acceptance
remained open after two shakedowns. The b7627fc rerun proved the code-stage
patch-quality gate could fire but exposed stale proposal-session refs; cdb8f43
attempted to fix that path, but the formal WSL rerun at
`/home/clawd/research/scion-experiments/v04-warehouse-validation-transfer-patch-quality-rerun6r-20260616T202258Z`
still paired a code-stage `warehouse_validation_transfer_patch_quality_missing`
failure detail with an earlier `partial_hypothesis_only` session ref. That
failed research-quality acceptance because agent feedback and postrun analysis
would receive mismatched lineage. The current local repair clears the
explore-step session-ref cache after code generation fails and normalizes
quality-block details to avoid repeated `agent_quality_blocked` prefixes.
Focused proposal, warehouse, feedback, and status tests pass locally. Next
gate: commit/push and relaunch one clean WSL warehouse `6R` patch-quality field
check.

The follow-up repair was committed as `6e13b11` (`fix: refresh code quality
session refs`) and pushed to `codex/v04-evidence-repair-plan`. A clean WSL
warehouse patch-quality field rerun was launched at
...
syncing to
`/home/clawd/research/scion-experiments/v04-warehouse-validation-transfer-patch-quality-rerun6r-6e13b11-20260616T203530Z`,
with tmux session `scion_wh_patchqual_rerun6r_6e13b11_203530`.

On 2026-06-17, the main session completed the code-stage behavior analysis for
the `a1dba41` warehouse helper-guard rerun. Subagent Ptolemy and main-thread
artifact replay agreed that the nine code-stage
`warehouse_validation_transfer_patch_quality_missing` blocks were primarily a
warehouse detector false negative, not just agent prose claiming a missing
guard. The blocked patches contained executable candidate-loop split/cost
filters that skipped dominated candidates with `continue` and returned the
original solution when no candidate was accepted. The prior detector accepted
direct `return solution` guards and helper `return None` guards but missed this
direct candidate-filter pattern. The local repair remains problem-owned in
`WarehouseDeliveryAdapter`: accept those direct candidate-filter guards while
still rejecting split-only, string/comment-only, local-only, and missing-
diagnostics shapes. Acceptance passed warehouse target preview (`40 passed`),
agentic/proposal suites (`64 passed`), telemetry/session preview suites
(`48 passed`), py_compile, and `git diff --check`. Next gate: commit and run a
single short local warehouse `6R` acceptance check on the server; keep WSL for
larger parallel matrices.

That field gate was committed and pushed as `2774afd` (`Accept warehouse
candidate filter guards`) and run locally at
...
The run completed cleanly with wrapper `exit_code=0`,
`run_validity.status=valid`, `6/6` effective rounds, `8` screening metric rows,
`0` validation/frozen rows, and no promotion. It field-accepted the detector
repair because the repeated `screening_or_lexicographic_guard` false positive
did not recur; the only code-stage warehouse patch-quality block was
`missing=bounded_candidate_policy`. Research quality remains rejected. The
most actionable remaining branch is `merge_vehicles`, which is weak-positive
with W/L/T `7/1/6`, median delta `775.0`, and allowed next actions `repair` /
`telemetry_wiring`, but telemetry reports
`activation_missing_or_wiring_suspect`. Next work should analyze that branch's
patch/runtime telemetry path and the remaining transfer-risk, bounded-candidate,
and branch-lesson semantic quality blocks before any broad WSL matrix.

The branch-level analysis confirmed the final `merge_vehicles` patch exported
activation and cost-effect diagnostics correctly: activation/activity fields
were positive, `cost_delta_sum` and `improving_move_count` were positive, and
only protected `split_delta_sum` stayed zero. The direct gate reason remained
screening quality (`7/1/6`, win rate `0.5`, `ci_low=0.0`), so the repair did
not change Decision or validation entry. The focused local repair separates
hard repairable telemetry failures from nonblocking effect-zero diagnostics:
`TELEMETRY_EFFECT_ZERO_DIAGNOSTIC` now renders as `telemetry_effect_zero`
without forcing `activation_missing_or_wiring_suspect` repair focus or hiding
the active weak-positive branch workspace. Acceptance passed branch/finalizer/
proposal prompt tests (`68 passed`), evaluation/telemetry tests (`47 passed`),
cross-branch/context tests (`25 passed`), py_compile, and `git diff --check`.
Next gate is one short local warehouse field check on the 2-core server; WSL
remains reserved for larger parallel matrices after that local signal is clean.

The local field check ran from commit `8688ac9` at
`/home/clawd/research/scion-experiments/v04-warehouse-effectzero-guidance-rerun6r-8688ac9-20260617T105940Z`
and completed with wrapper `exit_code=0`. The run was valid and complete at
`6/6`, produced `7` protocol metric rows (`6` screening, `1` validation), no
frozen row, no promotion, and `6` proposal quality blocks. It field-accepted
the targeted screening effect-zero repair: screening rows with
`TELEMETRY_EFFECT_ZERO_DIAGNOSTIC`, passed telemetry guards, and observed
activation progressed through `expand_screening` and then `queue_validate`
instead of being routed directly to telemetry wiring repair. Research quality
remains rejected. The validation row failed with real hard telemetry repair:
`swap_orders.accepted_moves`, `cost_delta_sum`, `improving_move_count`, and
`split_delta_sum` were all zero on validation, producing
`VALIDATION_TELEMETRY_REPAIRABLE` and `TELEMETRY_VALIDATION_REPAIRABLE`. The
next gate is focused warehouse proposal-quality repair, starting with the
blocked `change_vehicle_type.py` exact-replace session
`f7851de0-0fee-4420-b20d-3c27df9bfd73`; do not launch a broad WSL matrix from
this state.

The blocked `change_vehicle_type.py` session was classified as a true quality
block. The recovered patch only filtered on `cost_delta <= 0`; it did not
compute `split_delta`, `base_splits`, `candidate_splits`, or an equivalent
lexicographic objective comparison. The warehouse detector did not miss a
valid guard. A problem-owned feedback/skeleton repair is now locally accepted
and recorded at
`../experiments/v0.4/v04-warehouse-quality-skeleton-repair-20260617.md`. The
repair strengthens retry feedback with an executable split/cost guard skeleton,
`change_vehicle_type` / downsize cost-only warning, computed-delta diagnostics,
and real pre-evaluation bounded-candidate guidance. It does not relax
`_patch_has_screening_or_lexicographic_guard`, Decision, or
`DecisionFeatures`. Acceptance passed warehouse target preview (`41 passed`),
proposal-quality/recorder tests (`74 passed`), py_compile, and
`git diff --check`.

Resource check: no server or WSL Scion experiment process is active. WSL SSH is
healthy at `127.0.0.1:2222`, with `10` logical CPUs and about `27Gi` memory,
but the WSL checkout was still at `4b2ee29` and must be fast-forwarded before
new WSL experiments. Next gate after commit: one short local warehouse `6R`
field check on the 2-core server; reserve WSL for synchronized larger
matrices.

That short local field check was launched from commit `9853dd4`. Launch report:
...
check passed: `status=running`, campaign log reached
`Starting campaign: warehouse_delivery`, and early campaign artifacts exist.
An earlier `nohup` wrapper attempt at
`/home/clawd/research/scion-experiments/v04-warehouse-quality-skeleton-rerun6r-9853dd4-20260617T115932Z`
was marked `aborted_wrapper_no_campaign`; it created no campaign directory or
log and consumed no LLM/protocol work.

The `9853dd4` field gate then stopped early with API balance exhaustion:
...
`protocol_metric_results=2`, `formal_candidate_artifact_count=3`, and champion
stayed v1. Postrun:
`../experiments/v0.4/v04-warehouse-quality-skeleton-rerun6r-postrun-20260617.md`.
The run is not a completed field gate and is not warehouse efficacy evidence.
It is useful partial diagnostic evidence: the strengthened repair skeleton
reached code prompts, one late `change_vehicle_type.py` block was a true
quality block, and another was a warehouse detector false negative.

Curie classified `a1931a64-aa3a-4704-9b0e-846994ed0cb7` as a true block: the
patch added diagnostics and ranked candidates but did not compute executable
base/candidate split and cost comparisons. Curie classified
`c0e6a00c-0b86-42bb-b8c5-2e9a07d395be` as a detector false negative: the patch
computed base/candidate splits and costs, used sequential executable filters
`if split_delta < 0: continue` and `if cost_delta <= 0: continue`, and returned
the original solution when no candidate passed. The adapter only recognized a
single guard expression containing both metrics.

A narrow warehouse-owned detector repair is now locally accepted. The adapter
...
local-only diagnostic shapes. Acceptance passed warehouse target preview
(`42 passed`), proposal-quality/recorder tests (`74 passed`), py_compile, and
`git diff --check`. No Decision, Protocol threshold, or `DecisionFeatures`
change was made. Do not launch another LLM field gate until API balance is
restored; once restored, rerun one short local warehouse `6R` gate from the
repaired commit before any WSL matrix. WSL remains the right place for larger
parallel matrices; one or two short cells can run on the 2-core server.

The API/proxy blocker was rechecked and cleared for the next local gate: a
minimal `gpt-5.5` structured-tool call returned `LLM_OK`. WSL SSH was healthy
after restart, the WSL runner was fast-forwarded to `6921f70`, and WSL conda
`scion` plus `/v1/models` preflight passed. A short local warehouse production
`6R` field gate was launched from commit `6921f70`. Launch report:
`../experiments/v0.4/v04-warehouse-sequentialguard-rerun6r-launch-20260617.md`.
Run root:
...
`agentic_sessions/`, `status.json`, and `scion.db` artifacts exist. This is a
single-cell server gate; WSL is reserved for larger synchronized matrices.

The later copied-config data-root fallback field gate completed validly and
reached validation. It field-accepted the data-root fallback but not research
quality: the `swap_orders` validation row failed normally with
`VALIDATION_FAIL_NO_HIERARCHICAL_GAIN`. A focused audit recorded in
`../experiments/v0.4/v04-warehouse-swaporders-cvrp-focused-diagnostic-prep-20260617.md`
classified this as real mechanism insufficiency rather than infra, telemetry,
or generic gate failure: validation pair W/T/L was `8/1/6`, median delta was
`-200`, `split_delta_sum` stayed `0`, and median runtime ratio was about
`1.025`. The repair stayed problem-owned by exposing this aggregate no-gain
pattern in warehouse proposal diagnostics and order-level guidance while
excluding it from `DecisionFeatures`.

The same slice prepared the next CVRP no-LLM diagnostic by adding repeatable
`--case-id` support to `scion/tools/cvrp_mechanism_matrix.py`, so synchronized
WSL runs can target `P-n76-k4`, `P-n101-k4`, `M-n151-k12`, `CMT2`, and `CMT4`
without rerunning a full 16-case matrix. Local acceptance for the warehouse
guidance and CVRP exact-case tool passed targeted tests (`48 passed`) and
py_compile.

The focused exact-case WSL matrix from commit `70dfc53` then completed validly:
`../experiments/v0.4/v04-cvrp-focused5-mechanism-wsl-70dfc53-postrun-20260617.md`.
It ran `60/60` rows over `P-n76-k4`, `P-n101-k4`, `M-n151-k12`, `CMT2`, and
`CMT4` with four seeds. It rejected `alns_only` and
`size70_two_opt_candidate` as broad replacements (`4/15/1` overall vs
canonical, median delta `7.0`). The only replicated positive local signal was
`P-n76-k4` at `3/4` wins and median delta `-4.0`, with no route/fleet
regression. High-gap cases, especially `CMT2`, got worse without canonical
VNS, so the next CVRP action is deeper `P-n76-k4` fixed-candidate/no-LLM seed
and phase-telemetry diagnostics rather than a broad LLM campaign.

That deeper `P-n76-k4` seed diagnostic from commit `14c2a34` then completed
validly:
`../experiments/v0.4/v04-cvrp-p76-deepseed-wsl-14c2a34-postrun-20260617.md`.
It ran `60/60` rows over seeds `1..20` and rejected the earlier local-win
pocket as unstable: both `alns_only` and `size70_two_opt_candidate` were
`6/14/0` versus canonical with median delta `+8.0`. This blocks a P-n76-based
CVRP LLM campaign. The next CVRP step is variant separation and telemetry:
initial-VNS disabled, embedded-VNS disabled, current size70/two-opt fallback,
pure ALNS/no polish, and VNS before/after objective progress.

The warehouse side then received a local problem-owned acceptance-contract
guidance repair:
`../experiments/v0.4/v04-warehouse-validation-transfer-acceptance-contract-repair-20260617.md`.
It strengthened order-level/swap-style prompt and active subject code
constraints so split-positive moves are preferred and split-preserving
cost-only moves require computed `split_delta == 0`, `cost_delta > 0`,
exported `split_delta_sum`/`cost_delta_sum`, and a no-op fallback. This did not
change generic Decision, Protocol thresholds, validation/frozen gates, or
`DecisionFeatures`. Local acceptance passed warehouse target preview
(`42 passed`) and py_compile.

The CVRP side then received a local VNS variant/telemetry diagnostic repair:
`../experiments/v0.4/v04-cvrp-vns-variant-telemetry-repair-20260617.md`.
Default mechanism matrices remained the existing three mechanisms, while
explicit `--mechanism` selection gained `initial_vns_disabled`,
`embedded_vns_disabled`, and `pure_alns_no_polish`. The scheduler now records
objective probes around VNS phases so no-LLM matrices can attribute VNS timing
to objective progress. Local acceptance passed mechanism-matrix tests
(`8 passed`), py_compile, default dry-run job-count check (`3`), and a
3-second P-n76 smoke (`4/4` completed with objective probes).

The WSL CVRP VNS variant matrix from commit `6d742c6` then completed validly:
`../experiments/v0.4/v04-cvrp-vns-variant-matrix-wsl-6d742c6-postrun-20260617.md`.
It ran `80/80` rows and again rejected broad VNS removal / pure ALNS. The
useful signal became conditional initial-VNS scheduling:
`initial_vns_disabled` improved `CMT2` at `3/1/0`, median `-54.5`, and
`P-n76-k4` at `3/1/0`, median `-8.0`, but regressed slightly on `CMT4` and
`M-n151-k12`. The next CVRP step is a deeper canonical-vs-initial-VNS-disabled
seed matrix and objective-probe analysis for a case-general trigger.

That deeper canonical-vs-`initial_vns_disabled` WSL matrix from commit
`6d742c6` then completed validly:
`../experiments/v0.4/v04-cvrp-initial-vns-deepseed-wsl-6d742c6-postrun-20260617.md`.
It ran `160/160` rows and rejected disabling initial VNS as a stable standalone
mechanism: overall `25/51/4`, median delta `+2.0`; CMT2 `8/12/0`, median
`+3.5`; P-n76 `9/8/3`, median `0.0`. Objective probes showed initial VNS gain
is constant within each case, so the remaining CVRP opportunity is a richer
early-ALNS starvation or budget-pressure scheduling hypothesis, not a simple
initial-VNS disable rule.

A warehouse validation-transfer acceptance-contract short behavior gate was
then launched from commit `ce5d884`:
`../experiments/v0.4/v04-warehouse-validation-transfer-contract-rerun6r-launch-20260617.md`.
It runs on WSL at
...
and tmux session `scion_wh_vt_contract_rerun6r_ce5d884_152944`. Shape: one
warehouse production `6R` cell, `rep01/full_context`, measurement governance
`on`, disabled early stop, full proposal context, WSL-local `gpt-5.5`, and no
stage-drain/retry/session-timeout override. Initial health check passed and
early campaign artifacts exist.
