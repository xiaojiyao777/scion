...
It uses `--parallelism 4`, `--timeout-padding-sec 600`, and no LLM/APS calls.
Initial synced output has three completed `X-n401-k29` rows, all distance
`68673`, BKS gap `3.81%`, and `best_update_count=0`; final analysis is pending
run completion and sync of all 36 rows.
