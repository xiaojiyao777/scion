...
same-mechanism chain 5 in two repeats. However, mechanism family stayed
concentrated in `solver_design`, and postrun artifacts do not yet prove that
branch lessons changed later proposal choices. Prompt sampling showed code-phase
source visibility held, while hypothesis/code contexts remain large and
research/lesson sections still truncate.
...
CVRP work should inspect the ALNS-only validation-to-frozen collapses, add
report-only structured lesson-use accounting, and tighten hypothesis context
before another longer campaign.
