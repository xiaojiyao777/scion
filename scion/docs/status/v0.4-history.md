...
Proposal-visible context ablation is implemented and accepted as the next
Phase 5 control surface. `scion run --proposal-context-ablation` supports
`full`, `no-measurement-diagnostics`, and `minimal-research-context`; all three
arms keep `measurement_governance=on` so protocol/runtime/lifecycle/Decision
semantics are matched. The switch only affects tainted hypothesis prompt
visibility. `no-measurement-diagnostics` hides compact measurement diagnostics
and measurement-owned opportunity hints while retaining broader research
context. `minimal-research-context` hides broader branch/cross-branch/runtime/
opportunity research context while retaining source/champion/branch code and
compact measurement self-diagnosis. Prompt manifests and trajectory
fingerprints record the ablation mode. Acceptance: context/control/trajectory
suite `23 passed`, proposal pipeline/source-visibility suite `46 passed`,
CLI/config suite `31 passed`, py_compile, and `git diff --check`. The next
experiment is a warehouse 3-arm shakedown; CVRP remains excluded from formal
