...
mid-run prompt manifests, but exposed a second feedback-quality issue: generic
agentic rejection projection could overwrite the problem-owned warehouse retry
constraint with generic novelty text. The follow-up repair preserves the
structured retry constraint when present and carries `missing_claims` /
`missing_code_elements` forward into quality feedback, keeping actionable
warehouse patch requirements visible without adding warehouse semantics to
generic Decision.
