...
measurement change. Caveat: `CMT4` has file dimension `151` but used the
default `30s`, so exact protocol-time fidelity needs CMT dimension handling
repair or explicit pre-registration.
