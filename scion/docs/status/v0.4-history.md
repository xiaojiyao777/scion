...
Interim evidence from the active WSL run is pre-Protocol. It has
...
found code-stage source visibility was present, but prompt context remained
large and diluted by governance/rules content. The repeated hard runtime
failure was traced to a CVRP problem-owned boundary bug: ALNS/VNS scheduler
best-update instrumentation passed internal `_Solution` objects to
`record_best_update()`, but runtime audit coercion could not convert internal
...
claim. The active WSL 1R run remains pre-repair evidence because it launched
from commit `17fdeb8`; after it finishes, sync/analyze it as behavior debug
evidence and rerun a small one-round debug from the repaired commit before any
longer CVRP campaign.
