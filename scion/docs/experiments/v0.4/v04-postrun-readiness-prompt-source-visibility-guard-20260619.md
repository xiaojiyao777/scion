...
visibility accounting, so branch transfer, target-source grounding, and code
context visibility can be audited instead of inferred from final status.
...
current-run evidence, and to include nonzero aggregate trace and source
visibility trace counts.
...
Results: focused postrun readiness group `8 passed`; full v0.4
readiness/reporting group `87 passed`.
...
- actionable warehouse and CVRP summaries pass when current-run prompt/source
  visibility trace accounting is present; and
