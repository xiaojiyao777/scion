...
- CVRP now exposes `solver_design` as the preferred problem-owned
  problem-object solver-design surface. It is backed by the stable
  `policies/baseline_algorithm.py::solve(...)` entrypoint and focused
  branch-owned modules under `policies/baseline_modules/`; the older
  `policies/solver_algorithm.py::solve(...)` hook is compatibility only.
  Scion core sees only declared surface metadata and generic required runtime
  fields, not CVRP construction modes, route edits, destroy/repair,
  recombination, acceptance, restart, perturbation, or runtime-scheduling
  semantics.
