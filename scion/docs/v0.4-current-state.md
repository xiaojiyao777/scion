...
- CVRP formal research needs an implementation slice for problem-owned
  algorithm surfaces. `search_policy` proved useful as a generic surface model
  but too narrow as an optimization lever because it still acts around the
  post-baseline operator layer.
