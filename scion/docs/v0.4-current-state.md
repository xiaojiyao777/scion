...
These audit findings remain open after the first post-run repair pass.

P0:

- Frozen holdout usage budget must be consumed and enforced at campaign level;
  `FrozenConfig.max_uses_per_campaign` is currently configuration-only.
- Final evidence refs must become a formal campaign close/readiness condition,
  not only optional summary metadata.

P1:

- Remaining warehouse-shaped legacy fallbacks in framework verification,
  protocol evaluation, proposal taxonomy, and search memory should be moved
  behind problem adapters or made explicitly legacy-only.
- Programmatic `CampaignManager(adapter, experiment_protocol)` construction
  should inherit the protocol runner for verification gates so runtime checks
  are not skipped outside the CLI path.
- Static complexity governance should reject more unbounded operator families
  than the current high-order `combinations` checks.
