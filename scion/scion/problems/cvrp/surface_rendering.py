...
def render_problem_object() -> str:
    return (
        ...
        "state, and solver-time-limit polling.\n"
...
def render_solver_mechanics() -> str:
    return (
        ...
        "and validates its returned solution. If the active package fails to "
        "load or returns invalid output, the shell reports the runtime error and "
        "emits a nearest-neighbor fallback so downstream objective checks can run.\n"
...
def render_research_surface_interface(surface_name: str) -> str:
    ...
    return (
        ...
        "and rejects malformed, capacity-infeasible, or "
        "duplicate/missing-customer outputs. Excess route count is reported as "
        "fleet_violation and evaluated separately.\n\n"
        "Objective semantics:\n"
        "- fleet_violation = max(0, len(routes) - route_limit), where "
        "route_limit is instance.allowed_routes or instance.bks_routes.\n"
        ...
        "Do not pass arbitrary keyword arguments to these helpers.\n\n"
