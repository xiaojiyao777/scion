...
class CvrpSolverDesignProvider:
    ...
    def solver_design_target_api_guidance(self, target_file: str) -> str:
        ...
        if normalized == "policies/baseline_modules/seed_selector.py":
            return (
                "Target-specific rule for seed_selector.py: implement a cohesive "
                "post-construction seed selector module. Keep scheduler.py as "
                "minimal construction-boundary integration through typed "
                "additional_changes. Record seed_post_optimization_selector "
                "activation, phase runtime, and selected-seed-versus-baseline "
                "record_move delta before downstream ALNS/VNS. Do not move "
                "ALNS/VNS, q scheduling, acceptance, destroy/repair selection, "
                "or operator-credit policy into this module."
            )
        ...
        if normalized == "policies/baseline_modules/acceptance.py":
            return (
                ...
                "tie objective claims to formal per-case total_distance "
                "outcomes and guard allow/reject trajectory evidence. Put the "
                "guard helper and mechanism-specific activation/activity/budget "
                "telemetry in acceptance.py. Scheduler.py may pass "
                "candidate/current/best distances and consume the returned "
                "allow/reject decision, but scheduler.py must not call "
                "`context.record_move` for this acceptance mechanism. Do not "
                "credit ordinary current-improving or new-best ALNS/VNS "
                "bookkeeping to the guard, and do not reimplement rank-gap, "
    ...
    def solver_design_code_rules(self, context: Any) -> Sequence[str]:
        return (
            ...
            (
                ...
                "guard and mechanism-specific activation/activity/budget "
                "telemetry in acceptance.py. Scheduler.py wiring may call the "
                "helper after repair/VNS and before current-state commit, but "
                "scheduler.py must not directly record effect telemetry for "
                "this mechanism. Do not declare broad-loop "
                "`phase_best_delta`/`phase_improvement_counts` as successor44 "
                "effect; use formal per-case total_distance outcomes plus "
                "guard allow/reject trajectory evidence for objective claims."
            ),
            (
