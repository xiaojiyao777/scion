    ...
    def solver_design_hypothesis_guidance(self, context: Any) -> Sequence[str]:
        return (
            ...
            (
                ...
                "adaptive embedded-VNS share-70 cadence-2 opportunity below; "
                "refine it only with explicit objective, budget, cumulative "
                "embedded-VNS runtime-share, or repaired-candidate trigger "
                "evidence. Otherwise prefer a non-scheduler "
                "mechanism module or a stable-entrypoint algorithm-body change."
            ),
            (
                "Current CVRP no-LLM opportunity, proposal-only and excluded "
                "from DecisionFeatures/promotion gates: raw adaptive "
                "embedded-VNS cadence-2 lowered embedded-VNS runtime pressure "
                "but lost CMT4/X quality in follow-up checks. The fixed "
                "share-70 cadence-2 diagnostic protected the first ALNS "
                "embedded-VNS call, preserved CMT4/X-n110 as neutral in a "
                "64-row focused WSL matrix, improved P-n65 mean delta to -1.5, "
                "and finished overall at W/L/T 2/3/11, mean paired delta -0.12, "
                "median 0.0, with lower embedded-VNS share than early-8. Treat "
                "this as a refinement target, not as a default solver change."
            ),
            (
                "If using that adaptive-VNS opportunity, refine the trigger with "
                "cumulative embedded-VNS runtime-share, objective, remaining-"
    ...
    def solver_design_target_intent_guidance(self, context: Any) -> Sequence[str]:
        return (
            ...
            (
                ...
                "embedded-VNS share-70 cadence-2 trigger refinement. Unless "
                "an active same-branch constraint or explicit evidence rules "
                "it out, select `policies/baseline_modules/scheduler.py` as "
                "the owner target and use a mechanism id such as "
                "`adaptive_embedded_vns_share70_trigger` or "
                "`adaptive_embedded_vns_trigger_refinement`."
            ),
            (
                "A non-scheduler target is acceptable only when the selected "
                "target-intent notes give concrete evidence that share-70 "
                "cadence-2 trigger refinement is not the right owner for the "
                "current bottleneck. Do not default to another local-search "
                "operator solely because it is concrete or easy to implement."
            ),
            (
                "For the share-70 cadence-2 refinement, use cumulative "
