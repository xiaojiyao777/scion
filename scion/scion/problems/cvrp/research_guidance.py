...
REVIEWED_SUCCESSOR_GUIDANCE_LINE = (
    ...
    "scheduler causal path. Successor24 then reviewed "
    ...
    "construction seed gains were not preserved by downstream search. Prefer "
    f"the prepared successor26 clean-fork `{SUCCESSOR26_MECHANISM_ID}` next; "
    "it is a short-horizon seed trajectory selector owned by "
    f"`{SUCCESSOR26_TARGET_FILE}`, not a hard required mechanism."
)
...
NEXT_REQUIRED_DIRECTION = (
    ...
    "selection as reviewed/default-avoid evidence. For successor26 solver "
    "research, clean-fork to a materially different CVRP-owned causal path; "
    f"the current prepared solver slot selects `{SUCCESSOR26_MECHANISM_ID}` "
    "as a short-horizon seed trajectory selector that compares a bounded "
    "post-seed local-search trajectory before full downstream ALNS/VNS can "
    "blur causality. It is owned by "
    f"`{SUCCESSOR26_TARGET_FILE}`; construction edits should be limited to "
    "exposing existing seed candidates if needed. This prepared focus does "
    "not make the mechanism hard-required; `required_mechanism_ids` remains "
    "empty. "
    "Use "
    ...
    "string removal, insertion-cost lookahead repair, and savings seed "
    "selection and carries direct per-case objective-effect evidence."
)
CURRENT_QUESTION = (
    ...
    "successor24 insertion-cost lookahead repair, and successor25 raw "
    "construction seed-baseline selection, were reviewed without "
    "positive-at-MDE solver effect, can successor26 clean-fork to "
    f"`{SUCCESSOR26_MECHANISM_ID}`, a short-horizon seed trajectory selector, "
    "that improves total_distance with direct same-run baseline versus "
    "selected post-trajectory objective evidence and without repeating prior "
    "default-avoid families?"
)
REQUIRED_EVIDENCE = (
    (
        f"for the prepared successor26 design, live target-intent or hypothesis "
        f"names `{SUCCESSOR26_MECHANISM_ID}` with target_file "
        f"`{SUCCESSOR26_TARGET_FILE}` or records why the clean-fork changed "
    ...
    (
        f"`{SUCCESSOR26_MECHANISM_ID}` telemetry, if pursued, includes "
        "mechanism-id activation/runtime and direct same-run baseline versus "
        "selected short-horizon post-trajectory total_distance delta before "
        "downstream ALNS/VNS; it must also report feasibility, route count, "
        "candidate count, and strict budget/reserve evidence"
    ),
    (
    ...
    (
        ...
        "cw_sweep_seed_baseline_selector successor25 evidence, and prior "
    ...
    (
        ...
        "not preserved downstream; successor26 solver research should "
        "clean-fork to another materially different CVRP-owned causal path, with "
        ...
        "and exact_short_route_polish were reviewed no-positive-at-MDE and "
...
MEASURABLE_OPPORTUNITY_CLASSES = (
    (
        ...
        "is now reviewed/default-avoid because direct seed gains were not "
        "preserved downstream; current focus is "
        f"`{SUCCESSOR26_MECHANISM_ID}` in `{SUCCESSOR26_TARGET_FILE}`, comparing "
        "a small set of existing construction candidates after a strictly "
        "bounded short-horizon trajectory before full ALNS/VNS. Successor18b "
        "moved granular_savings_seed_portfolio to reviewed "
        "below-MDE evidence and exact_short_route_polish to reviewed loss-heavy "
        "evidence, so do not continue unchanged variants; do not repeat "
...
SUCCESSOR_PORTFOLIO_RULE = (
    ...
    "cw_sweep_seed_baseline_selector have repeated it as construction "
    "successors, and operator_pair_destroy_size_bands plus "
    ...
    "seed deltas were not preserved downstream. The next CVRP solver slot "
    "should clean-fork to a materially different problem-owned causal path; "
    "current successor26 focus is short-horizon seed trajectory selection with "
    "direct same-run post-trajectory attribution. The same scheduler "
