...
Generic Contract dispatches here through the CVRP adapter. The check remains
static and conservative: new module-level helper functions must be reachable
from a module entrypoint or from the runtime solver class's ``solve`` call chain
inside the same candidate patch. Solver modules may also pass functions as
first-class operators, for example from ``_default_vns_operators()`` into
``_vns(...)``; those name references count as reachability edges even when the
helper is not called at definition time.
"""
