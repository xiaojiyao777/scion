...
HYPOTHESIS_TOOL: Dict[str, Any] = {
    ...
    "description": (
        ...
        "- When the selected surface declares mechanism telemetry, include mechanism_changes with the specific mechanism id(s) and generic change_type values: add, modify, replace, remove, or integrate. Branch action labels such as tune, repair, parameterize, and telemetry_wiring are research action labels, not mechanism_changes[].change_type values; map tune/parameterize to modify and telemetry_wiring to modify or integrate.\n"
...
PATCH_TOOL: Dict[str, Any] = {
    ...
    "description": (
        ...
        "- Echo the approved hypothesis mechanism_changes ids exactly when present, and use only legal generic change_type values. Do not write branch action labels such as tune, parameterize, or telemetry_wiring as mechanism_changes[].change_type.\n\n"
        ...
        "positive deltas and do not self-reject with a contradicted premise; "
        "record natural activity, decision, skipped, or budget diagnostics and "
        "let validation classify missing effect evidence.\n"
