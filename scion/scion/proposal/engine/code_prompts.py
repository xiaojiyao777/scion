...
def _split_code_context(
    ...
    user_prompt = (
        ...
        f"- Use `contradicted` only for a direct false premise against visible evidence, and `wrong_owner` only when a different owner file must change; leave patch fields empty for those two hard blocks.\n"
        f"- Use `duplicate` only as a diagnostic when visible code already has a close mechanism. If the approved hypothesis is still a material variant, continue with the patch and explain the overlap in `premise_check_reason`.\n"
        ...
        f"edits. Do not emit conflicting `full_file` entries for one file.\n"
