...
def _split_code_context(
    ...
    user_prompt = (
        ...
        f"- Use `contradicted` only for hard boundary/objective-policy/protected-constraint contradictions against visible evidence, and `wrong_owner` only when a different owner file must change; leave patch fields empty for those hard blocks.\n"
        f"- Do not use `contradicted` for novelty, duplicate-risk, near-existing mechanism, or baseline-already-has-similar-capability observations.\n"
        f"- Use `duplicate` only as a diagnostic when visible code already has a close mechanism. If the approved hypothesis is still a material variant, continue with the patch and explain the overlap in `premise_check_reason`; do not change direction merely to satisfy novelty wording.\n"
