...
def _split_direct_v3_hypothesis_context(
    ...
    user_prompt = (
        ...
        "Treat mechanisms already evaluated in experiment_history as the current "
        "research frontier. Before proposing, compare against that frontier. "
        "Reusing an evaluated target is allowed only for a concrete incremental "
        "algorithmic delta that addresses an observed limitation; a semantically "
        "equivalent rewrite is not a refinement. Otherwise pivot to another "
        "evidence-grounded bottleneck. Identify the bottleneck; propose one "
        "evidence-grounded mechanism-level change or refinement. "
