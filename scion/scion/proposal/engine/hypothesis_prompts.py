...
def _split_hypothesis_context(
    ...
    user_prompt = (
        ...
        f"mechanism activity evidence, not objective/outcome fields such as "
        f"`solver_algorithm_fleet_violation`. For mapping telemetry, use the "
        f"mechanism-specific path such as `some_runtime_map.<mechanism_id>`; "
        f"the whole map field alone is not activation evidence.\n\n"
