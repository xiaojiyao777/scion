...
def _split_hypothesis_context(
    ...
    user_prompt = (
        ...
        f"such as `solver_algorithm_fleet_violation`. Good activation paths "
        f"look like `solver_algorithm_context_records.<mechanism_id>_iterations` "
        f"or `solver_algorithm_phase_runtime_ms.<mechanism_id>`. Aggregate "
        f"outcome/activity fields such as `solver_algorithm_improving_moves` "
        f"or `solver_algorithm_best_improving_moves` show effect or activity, "
        f"not activation. For mapping telemetry, use the mechanism-specific "
        f"path such as `some_runtime_map.<mechanism_id>`; the whole map field "
        f"alone is not activation evidence. Do not use existing phase names "
        f"such as `.vns` as activation for a newly declared mechanism unless "
        f"that exact mechanism id is declared and owned by `mechanism_changes`. "
        f"For changes inside an existing ALNS/VNS phase, name the changed lever "
        f"as a mechanism id and use that same id in every expected telemetry path; "
        f"do not declare one mechanism and then point activation/effect/budget at "
        f"generic `.alns` or `.vns` phase buckets.\n\n"
        f"Objective field contract: `target_objectives` and "
        f"`protected_objectives` must contain only declared problem objective ids. "
        f"Hard constraints, feasibility conditions, capacity validity, parser "
        f"validity, and route consistency belong in `risk_to_higher_priority` "
        f"or `no_op_condition`, not in objective arrays.\n\n"
