...
def _solver_design_telemetry_obligation_rule(mechanism_ids: list[str]) -> str:
    ...
    return (
        ...
        "budget records for each declared id. duplicate/contradicted premise "
        "checks are diagnostic for mechanism overlap or premise concern; they "
        "are not a reason to omit telemetry for an implemented material "
        "variant. Only wrong_owner or explicit hard boundary/objective-policy/"
        "protected-constraint contradictions can skip implementation evidence. "
        "Do not only record a parent "
