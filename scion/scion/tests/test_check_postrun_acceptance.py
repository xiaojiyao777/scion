...
def test_postrun_acceptance_rejects_postrun_failure_markers_without_status_update(
    tmp_path: Path,
) -> None:
    run_root = _write_current_run_root(tmp_path / "run-root")
    (run_root / "exit.txt").write_text(
    ...
    assert "launcher_wrapper_marker_status_ok" in readiness["failed_required_checks"]
    assert marker_check["status"] == "failed"
    failures = marker_check["detail"]["failures"]
    assert "postrun_acceptance_failed_marker_present" in failures
    assert "postrun_reports_effective_exit_status_marker_present" in failures
    assert "postrun_readiness_effective_exit_status_marker_present" in failures
...
def test_postrun_acceptance_rejects_problem_summary_current_run_mismatch(
    tmp_path: Path,
) -> None:
    run_root = _write_current_run_root(
        tmp_path / "warehouse-run-stale-current-run-summary"
    )
    rebuild_tool.rebuild_postrun_acceptance(
        run_root,
        report_stem="fixture",
    ...
    readiness = check_tool.build_readiness(run_root)
    problem_check = readiness["checks"]["problem_summary_actionability"]
    detail = problem_check["detail"][0]

    assert readiness["current_run_analysis_ready"] is False
    assert readiness["checks"]["analysis_brief_current_run_evidence"]["status"] == (
        "failed"
    )
    assert problem_check["required"] is True
    assert problem_check["status"] == "failed"
    assert detail["current_run_evidence"] is True
    assert detail["expected_current_run_evidence"] is False
    assert "problem_summary_current_run_evidence_mismatch" in detail[
        "summary_failures"
    ]
    assert (
        check_tool.main([str(run_root), "--require-current-run-ready"])
...
def test_postrun_acceptance_rejects_present_stale_optional_review_input_summary(
    tmp_path: Path,
) -> None:
    run_root = _write_current_run_root(
        tmp_path / "warehouse-run-stale-optional-input"
    )
    rebuild_tool.rebuild_postrun_acceptance(
        run_root,
        report_stem="fixture",
    ...
    _add_prompt_source_visibility_summary(brief)
    _apply_warehouse_quality_blocked_review_inputs(brief, quality_block_count=2)
    measurement_summary = brief["measurement_effect_summary"]
    assert isinstance(measurement_summary, dict)
    measurement_summary["current_run_evidence"] = False
    brief_path.write_text(json.dumps(brief, indent=2, sort_keys=True), encoding="utf-8")

    readiness = check_tool.build_readiness(run_root)
    input_check = readiness["checks"]["review_input_summaries_actionability"]
    measurement_detail = next(
        item
        for item in input_check["detail"]["summaries"]
        if item["summary"] == "measurement_effect_summary"
    )

    assert readiness["current_run_analysis_ready"] is False
    assert input_check["status"] == "failed"
    assert measurement_detail["required_for_interpretation"] is False
    assert measurement_detail["current_run_evidence"] is False
    assert "measurement_effect_summary_not_current_run_evidence" in measurement_detail[
        "failures"
    ]
    assert (
        check_tool.main([str(run_root), "--require-current-run-ready"])
...
def test_postrun_acceptance_rejects_cvrp_ready_summary_with_stale_direct_evidence_detail(
    ...
    run_root = _write_current_run_root(tmp_path / "cvrp-run-stale-direct-detail")
    rebuild_tool.rebuild_postrun_acceptance(
        run_root,
        report_stem="fixture",
        observed_control_arm="on",
    ...
    assert consistency["status"] == "failed"
    failures = consistency["detail"]["failures"]
    assert "problem_summary_large_twoopt_direct_evidence_mismatch" in failures
    assert "review_input_large_twoopt_direct_evidence_missing" not in failures
    assert (
        consistency["detail"]["summary_large_twoopt_direct_evidence"][
            "activation_observed_count"
        ]
        == 99
    )
    assert (
        consistency["detail"]["input_large_twoopt_direct_evidence"][
            "activation_observed_count"
        ]
        == 1
    )
...
def test_postrun_acceptance_rejects_cvrp_ready_summary_with_cmt_route_only_input(
    tmp_path: Path,
) -> None:
    run_root = _write_current_run_root(tmp_path / "cvrp-run-cmt-route-only-ready")
    rebuild_tool.rebuild_postrun_acceptance(
        run_root,
        report_stem="fixture",
        observed_control_arm="on",
                ...
                "top_rows_by_effect_to_mde": [
                    {
                        ...
                        "case_level_total_distance_deltas": {
                            "CMT2": {
                                ...
                                "feasible_after": True,
                                "route_count_before": 8,
                                "route_count_after": 8,
                            },
                            "CMT4": {
                                "feasible_before": True,
                                "feasible_after": True,
                                "route_count_before": 12,
                                "route_count_after": 12,
                            },
                        },
                    }
                ]
    ...
    assert "review_input_large_twoopt_direct_evidence_missing" in failures
    assert (
        consistency["detail"]["input_large_twoopt_direct_evidence"]["missing"]
        == ["missing_cmt_case_protection_evidence"]
    )
    assert (
        consistency["detail"]["input_large_twoopt_direct_evidence"][
...
def test_postrun_acceptance_rejects_warehouse_plateau_ready_with_missed_same_mechanism(
    tmp_path: Path,
) -> None:
    run_root = _write_current_run_root(tmp_path / "warehouse-run-missed-followup")
    rebuild_tool.rebuild_postrun_acceptance(
        run_root,
        report_stem="fixture",
        observed_control_arm="on",
    ...
    assert "problem_summary_warehouse_continuity_substantive_mismatch" in failures
    assert "review_input_warehouse_continuity_not_substantive" in failures
    assert consistency["detail"]["summary_continuity_substantive"] is True
    assert consistency["detail"]["input_continuity_substantive"] is False
    assert (
