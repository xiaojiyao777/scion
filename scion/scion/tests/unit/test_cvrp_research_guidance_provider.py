...
def test_cvrp_legacy_research_focus_keeps_prepared_manifest_keys() -> None:
    ...
    assert focus["evidence_requirements"][0]["requirement_id"] == (
        "cvrp_algorithmic_intervention_sufficiency"
    )
    ...
    assert focus["successor46b_reviewed_evidence"]["target_file"] == (
        "policies/baseline_modules/best_solution_intensification.py"
    )
    assert "policies/baseline_modules/scheduler.py" in (
        focus["successor46b_reviewed_evidence"]["target_files"]
    )
