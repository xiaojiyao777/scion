...
def test_cvrp_research_guidance_contract_contains_required_blocks() -> None:
    ...
    assert "bounded_destroy_operator_shadow_selector_protected_followup" in (
        rendered.text
    )
    assert SUCCESSOR44_MECHANISM_ID in rendered.text
...
def test_cvrp_legacy_research_focus_keeps_prepared_manifest_keys() -> None:
    ...
    assert focus["successor43_reviewed_evidence"]["mechanism_id"] == (
        SUCCESSOR43_MECHANISM_ID
    )
    assert focus["successor43_reviewed_evidence"]["target_file"] == (
        "policies/baseline_modules/destroy_operator_selector.py"
    )
    assert "policies/baseline_modules/scheduler.py" in (
        focus["successor43_reviewed_evidence"]["target_files"]
    )
    assert focus["successor43_reviewed_evidence"]["status"] == (
        "reviewed_marginal_below_mde_protected_case_unsafe"
    )
    assert focus["successor43b_reviewed_evidence"]["mechanism_id"] == (
        SUCCESSOR43B_MECHANISM_ID
    )
    assert focus["successor43b_reviewed_evidence"]["target_file"] == (
        "policies/baseline_modules/destroy_operator_selector.py"
    )
    assert "policies/baseline_modules/scheduler.py" in (
        focus["successor43b_reviewed_evidence"]["target_files"]
    )
    assert focus["successor43b_reviewed_evidence"]["status"] == (
        "reviewed_below_mde_protected_case_unsafe"
    )
    assert SELECTOR_TELEMETRY_HYGIENE_LESSON in (
        focus["successor43b_reviewed_evidence"]["carried_lessons"]
    )
    assert focus["successor44d_reviewed_evidence"]["mechanism_id"] == (
        SUCCESSOR44_MECHANISM_ID
    )
    assert focus["successor44d_reviewed_evidence"]["target_file"] == (
    ...
    assert focus["successor46_reviewed_evidence"]["target_file"] == (
        "policies/baseline_modules/best_solution_intensification.py"
    )
    assert "policies/baseline_modules/scheduler.py" in (
        focus["successor46_reviewed_evidence"]["target_files"]
    )
    ...
    assert focus["successor46b_target_intent"]["mechanism_id"] == (
        SUCCESSOR46B_MECHANISM_ID
    )
    assert focus["successor46b_target_intent"]["target_file"] == (
        "policies/baseline_modules/best_solution_intensification.py"
    )
    assert "policies/baseline_modules/scheduler.py" in (
        focus["successor46b_target_intent"]["target_files"]
    )
    assert focus["successor46b_target_intent"]["required_mechanism_binding"] == (
        "target_intent_required"
    )
    assert "rejected-attempt RNG isolation evidence" in (
        focus["successor46b_target_intent"]["material_difference"]["evidence"]
    )
    ...
    assert "target-intent-bound to successor46b" in focus["next_required_direction"]
    ...
    assert best_solution_effect["screening_pair_wins"] == 4
    ...
    assert destroy_shadow_effect["row2_pair_wins"] == 33
    assert destroy_shadow_effect["protected_case_cmt2_median_delta"] == -5.5
    assert destroy_shadow_effect["recommended_followup"] == (
        "bounded_destroy_operator_shadow_selector_protected_followup"
    )
    ...
    assert protected_destroy_shadow_effect["combined_pair_wins"] == 42
    assert protected_destroy_shadow_effect["combined_pair_losses"] == 49
    assert protected_destroy_shadow_effect["protected_case_cmt2_delta_sum"] == -74
    assert protected_destroy_shadow_effect["recommended_followup"] == (
