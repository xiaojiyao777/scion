...
_R11 = "v04-cvrp-r11-wide-budget-autonomous"
...
def test_r11_preserves_all_wide_budget_rules_and_scientific_gates():
    old = yaml.safe_load(_path(_R10, "protocol.yaml").read_text())
    new = yaml.safe_load(_path(_R11, "protocol.yaml").read_text())
...
def test_r11_seed_selection_is_prospective_and_disjoint():
    ledger = SeedLedgerConfig.from_yaml(_path(_R11, "seeds.yaml"))
    seeds = [*ledger.screening, *ledger.validation, *ledger.frozen, *ledger.canary]
    primes = [
        n
        for n in range(100001, 100300)
    ...
    protocol = yaml.safe_load(_path(_R11, "protocol.yaml").read_text())
    assert protocol["canary"]["seeds"] == ledger.canary


def test_r11_h_receives_screening_facts_without_private_validation_details():
    old = json.loads(_path(_R9, "research-input.json").read_text())
    value = normalize_research_input(
        json.loads(_path(_R11, "research-input.json").read_text())
    )
    ...
    h = context.build_hypothesis_context(
        branch=Branch(
            branch_id="r11-input-check", state=BranchState.EXPLORE, base_champion_id=1
        ),
