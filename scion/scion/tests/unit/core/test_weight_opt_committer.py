...
def _committer(
    *,
...
) -> tuple[WeightOptCommitter, FakeEventSource, FakeRegistry, list[ChampionState]]:
    state = [champion]
    ...
    return committer, event_source, registry, state
...
def test_drain_commits_improved_weight_event_and_marks_branches_stale() -> None:
    ...
    champion = _champion()
    committer, source, registry, state = _committer(
        champion=champion,
        events=[event],
        branch_controller=ctrl,
    )
    ...
    assert state[0].promoted_at == champion.promoted_at
