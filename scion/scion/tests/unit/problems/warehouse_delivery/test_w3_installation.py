...
def test_generated_authority_input_rejects_duplicate_content_identity() -> None:
    duplicate = hashlib.sha256(b"duplicate input").hexdigest()
    with pytest.raises(
        WarehouseW3InstallationError,
        match="input_sha256 contains a duplicate",
    ):
        AuthorityInputAdapter.generated(
            logical_path="generated/object",
            sealed_path="sealed/generated/object",
            raw=b"generated bytes",
            generator_sha256=hashlib.sha256(b"generator").hexdigest(),
            input_sha256=(duplicate, duplicate),
            rule_sha256=hashlib.sha256(b"rule").hexdigest(),
        )
