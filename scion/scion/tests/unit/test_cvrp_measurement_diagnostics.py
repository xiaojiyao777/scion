...
def test_cvrp_adapter_renders_proposal_only_measurement_opportunities() -> None:
    ...
    assert "weak-positive below MDE" in ranking[0]["summary"]
    ...
    assert "SUCCESSOR35_CAPACITY_TIGHTNESS_READY" in rendered
...
def test_context_manager_projects_cvrp_adapter_opportunities_top_level() -> None:
    ...
    assert "SUCCESSOR35_CAPACITY_TIGHTNESS_READY" in compact
