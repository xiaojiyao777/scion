...
from scion.problems.warehouse_delivery import protocol_population as population_module
...
EXPECTED_PROTOCOL_SHA256 = (
    "6232b9653e260334864fa18afce5bc4513fb092925d2a50b6512bc61f1fb25ba"
)
...
def test_prod_population_reconcile_records_content_bound_selections() -> None:
    ...
    assert (
        ...
        payload["sources"]["protocol"]["sha256"],
    ...
    for request in payload["requests"]:
        ...
        assert all(
            case["content_sha256"]
            for case in request["selected_case_identities"]
        )


def test_protocol_and_manifest_parse_and_hash_the_same_single_read(
    tmp_path: Path,
    ...
    original_reader = population_module._read_regular_snapshot
    source_read_counts = {"protocol source": 0, "manifest source": 0}

    def mutate_after_snapshot(path: str | Path, **kwargs: Any):
        snapshot = original_reader(path, **kwargs)
        label = str(kwargs["label"])
        if label in source_read_counts:
            source_read_counts[label] += 1
        if label == "protocol source":
            protocol_path.write_text("version: mutated-after-read\n", encoding="utf-8")
        return snapshot
    ...
    assert source_read_counts == {"protocol source": 1, "manifest source": 1}
    assert payload["protocol_version"] == EXPECTED_PROTOCOL_VERSION
    assert payload["sources"]["protocol"]["sha256"] == hashlib.sha256(
        original_protocol
    ).hexdigest()
    assert payload["sources"]["protocol"]["sha256"] != hashlib.sha256(
...
def test_prepared_copy_rejects_case_content_drift(tmp_path: Path) -> None:
    source_protocol, source_manifest, _ = _write_small_assets(tmp_path / "source")
    prepared_protocol, prepared_manifest, prepared_cases = _write_small_assets(
        tmp_path / "prepared"
    )
    source = reconcile_warehouse_protocol_population_from_paths(
        source_protocol,
        source_manifest,
    )
    prepared_cases["screening"].write_text(
        '{"stage":"screening","drift":true}',
        encoding="utf-8",
    )
    prepared = reconcile_warehouse_protocol_population_from_paths(
        prepared_protocol,
        prepared_manifest,
    )
    ...
    with pytest.raises(
        WarehouseProtocolPopulationError,
        match="prepared-copy case population content drift",
    ):
        assert_warehouse_population_copy_matches(source, prepared)
