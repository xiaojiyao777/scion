...
def test_cvrp_agentic_launcher_prepare_writes_run_files(tmp_path: Path) -> None:
    ...
    assert measurement["top_opportunity_recipe"]["mechanism_id"] == (
        "materially_distinct_cvrp_owned_causal_path"
    )
    ...
    assert measurement["mechanism_effect_ranking"][0]["opportunity_status"] == (
        "open_materially_distinct_path_required"
    )
    ...
    assert measurement["mechanism_effect_ranking"][2]["opportunity_status"] == (
        "reviewed_default_avoid"
    )
    assert measurement["mechanism_effect_ranking"][2]["evidence_status"] == (
        "successor26b_seed_trajectory_below_mde"
    )
    ...
    assert "load_complement_pair_removal" in prepared_manifest_md
    ...
    assert resume_continuity["schema_version"] == (
        "scion.cvrp_resume_continuity_requirements.v1"
    )
    assert resume_continuity["proposal_visibility_only"] is True
    assert resume_continuity["decision_features_excluded"] is True
    ...
    assert "open_materially_distinct_path_required" in prepared_manifest_md
    ...
    assert "zero branch cards" in prepared_manifest_md
    ...
    assert (
        prepared_prompt_context["signals"]["cvrp_resume_continuity_requirements"][
            "available"
        ]
        is True
    )
