...
def test_cvrp_problem_spec_loads(
    cvrp_spec: ProblemSpecV1,
    cvrp_adapter: ProblemAdapter,
) -> None:
    assert cvrp_spec.id == "cvrp"
    assert [o.name for o in cvrp_spec.objectives] == [
        "fleet_violation",
        "total_distance",
    ]
...
def test_cvrp_solver_design_surface_interface_renders_safe_instance_api(
    ...
    assert "context.make_solution(routes)" in rendered
    assert "context.remaining_time()" in rendered
