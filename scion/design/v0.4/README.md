# Scion v0.4 Design Index

*Last updated: 2026-05-08*

These are active v0.4 design-source documents. Current implementation status is
tracked in [docs/status/current-state.md](../../docs/status/current-state.md).

## Core Design Sources

- [v0.4 design baseline](v0.4-design.md) - performance-aware Scion, CVRP generalization, and campaign framework cleanup.
- [Algorithm design-space upgrade](v0.4-algorithm-design-space-upgrade.md) - integrated design source for research surfaces v2 and AgenticProposalSession.
- [Algorithm design-space optimization](v0.4-algorithm-design-space-optimization.md) - motivation and evidence for moving beyond operator-only optimization.
- [Agentic proposal session design](v0.4-agentic-proposal-session-design.md) - bounded proposal-agent design.
- [CVRP research-surface design](v0.4-cvrp-research-surface-design.md) - first non-operator CVRP research surface design.
- [Problem and algorithm onboarding](v0.4-problem-algorithm-onboarding.md) - design source for turning human-written solvers into Scion-native research objects.

## Framework And Evidence Designs

- [CVRP plan](v0.4-cvrp-plan.md)
- [Evidence harness](v0.4-evidence-harness.md)
- [Performance-aware plan](v0.4-performance-aware-plan.md)
- [P0 campaign controller decomposition](v0.4-p0-campaign-controller-decomposition-design.md)
- [P0 promotion integrity](v0.4-p0-promotion-integrity-design.md)
- [P1 runtime-aware optimization](v0.4-p1-runtime-aware-optimization-design.md)

## Phase Designs

- [Phase 3 design](v0.4-phase3-design.md)
- [Phase 4 design](v0.4-phase4-design.md)

## Related Docs

- [v0.4 current state](../../docs/status/current-state.md)
- [v0.4 planning index](../../docs/planning/v0.4/README.md)
- [Framework code map](../../docs/engineering/framework-code-map/README.md)
