...
### v0.1 首轮验证：15r Campaign

SubcatMergeSafe 算子通过三级验证（Screening 95% → Validation 100% → Frozen 100%），Frozen md=5,150,000。10 个算子中 6 个被 Verification Gate 拦截（V5 state leak），3 个被统计拒绝，1 个晋升。
...
生产数据 splits≈0 时，优化退化为 cost-only，信号弱，权重优化失灵。评分函数需按问题特征适配（v0.3 backlog）。
